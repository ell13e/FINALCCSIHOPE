...
/**
 * Form emails: CCS_EMAIL_OFFICE and CCS_EMAIL_RECRUITMENT are defined in functions.php
 */
...
function ccs_handle_contact_form() {
    ...
    if (class_exists('\\CCS\\Controllers\\ContactFormController')) {
        $validator = new \CCS\Services\FormValidator();
        $controller = new \CCS\Controllers\ContactFormController($validator);
        $controller->handle();
        return;
    }
    ...
    try {
        ...
        $bot_check = ccs_validate_anti_bot('contact');
        if ($bot_check === false) {
        ...
        // Validate name
        $name_validation = ccs_validate_name($name);
        if (!$name_validation['valid']) {
            $errors['name'] = $name_validation['error'];
        }

        // Validate email
        $email_validation = ccs_validate_email($email, true);
        if (!$email_validation['valid']) {
            $errors['email'] = $email_validation['error'];
        }

        // Validate phone
        $phone_validation = ccs_validate_uk_phone($phone);
        ...
        if (!defined('CCS_EMAIL_OFFICE') || empty(CCS_EMAIL_OFFICE)) {
            error_log('CCS Contact Form: CCS_EMAIL_OFFICE constant not defined');
            wp_send_json_error([
        ...
        $saved = ccs_save_form_submission($submission_data, $enquiry_type, false, '');
        ...
        $body .= "IP: " . (function_exists('ccs_get_client_ip') ? ccs_get_client_ip() : '') . "\n";
        ...
        if (!$sent) {
            ...
            error_log('CCS Contact Form: Failed to send email');
            if (is_array($last_error) && isset($last_error['type']) && $last_error['type'] === E_ERROR) {
                error_log('CCS Contact Form: PHP Error - ' . (isset($last_error['message']) ? $last_error['message'] : 'Unknown error'));
            }
            
            // Check if wp_mail function exists
            if (!function_exists('wp_mail')) {
                error_log('CCS Contact Form: wp_mail function does not exist');
            }
        }
        ...
        if ($marketing_consent && !empty($email)) {
            ...
            ccs_add_newsletter_subscriber($email, ccs_get_client_ip(), $first_name, $last_name);
        }

        // If submission was saved successfully, show success even if email failed
        if (!is_wp_error($saved)) {
            // Log successful saves (ID only for privacy)
            error_log('CCS Contact Form: Submission saved (ID: ' . $saved . ') - Type: ' . $enquiry_type);
            
        if (defined('WP_DEBUG') && WP_DEBUG) {
                if ($sent) {
                    error_log('CCS Contact Form: Email successfully sent');
                } else {
                    error_log('CCS Contact Form: Email failed but submission saved (ID: ' . $saved . ')');
                }
            }
        } else {
            // Log save failures (error message only, no personal data)
            error_log('CCS Contact Form: FAILED to save submission - ' . $saved->get_error_message());
        }
 ...
 * =========================================
 */
function ccs_handle_newsletter_signup() {
    // Use controller if available, otherwise fall back to legacy implementation
    if (class_exists('\\CCS\\Controllers\\NewsletterSignupController')) {
        $controller = new \CCS\Controllers\NewsletterSignupController();
        $controller->handle();
        return;
    }
    ...
    // Comprehensive anti-bot validation
    $bot_check = ccs_validate_anti_bot('newsletter');
    if ($bot_check === false) {
        // Bot detected - silently accept but don't process
        wp_send_json_success([
    ...
    if (!empty($phone)) {
        $phone_validation = function_exists('ccs_validate_uk_phone') ? ccs_validate_uk_phone($phone) : ['valid' => true];
    ...
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log('CCS Newsletter: ' . $result);
    }
    ...
    $submission_id = ccs_save_form_submission($submission_data, 'newsletter', false, '');
    ...
    if (!is_wp_error($submission_id) && $submission_id) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('CCS Newsletter: Submission created (ID: ' . $submission_id . ')');
        }
    } else {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('CCS Newsletter: Failed to create submission');
        }
    }
...
add_action('wp_ajax_ccs_newsletter_signup', 'ccs_handle_newsletter_signup');
add_action('wp_ajax_nopriv_ccs_newsletter_signup', 'ccs_handle_newsletter_signup');
 ...
 * =========================================
 */
function ccs_handle_group_booking() {
    // Use controller if available, otherwise fall back to legacy implementation
    if (class_exists('\\CCS\\Controllers\\GroupBookingController')) {
        $controller = new \CCS\Controllers\GroupBookingController();
        $controller->handle();
        return;
    }
    ...
    try {
        ...
        // Comprehensive anti-bot validation
        $bot_check = ccs_validate_anti_bot('group-booking');
        if ($bot_check === false) {
            // Bot detected - silently accept but don't process
            wp_send_json_success([
    ...
    if (!defined('CCS_EMAIL_OFFICE') || empty(CCS_EMAIL_OFFICE)) {
        error_log('CCS Group Booking: CCS_EMAIL_OFFICE constant not defined');
        wp_send_json_error([
            'message' => 'Email configuration error. Please contact us directly.',
            'code' => 'email_config_error'
        ], 500);
    ...
    $saved = ccs_save_form_submission($submission_data, 'group-booking', false, '');

        $to = CCS_EMAIL_OFFICE;
    $subject = sprintf('[CCS Group Training] %s - %d staff', $organisation, $number_of_staff);
    ...
    if (!$sent) {
        $email_error = 'Email sending failed';
        $last_error = error_get_last();
        error_log('CCS Group Booking: Failed to send email');
        if (is_array($last_error) && isset($last_error['type']) && $last_error['type'] === E_ERROR) {
            error_log('CCS Group Booking: PHP Error - ' . (isset($last_error['message']) ? $last_error['message'] : 'Unknown error'));
        }
        
        // Check if wp_mail function exists
        if (!function_exists('wp_mail')) {
            error_log('CCS Group Booking: wp_mail function does not exist');
        }
    }
    ...
    if (!is_wp_error($saved)) {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            if ($sent) {
        error_log('CCS Group Booking: Successfully sent email');
            } else {
                error_log('CCS Group Booking: Email failed but submission saved (ID: ' . $saved . ')');
            }
    }
        ...
        error_log('CCS Group Booking: Exception - ' . $e->getMessage());
        wp_send_json_error([
            'message' => 'An error occurred while processing your request. Please try again or contact us directly.',
            'code' => 'server_error'
        ], 500);
...
add_action('wp_ajax_ccs_group_booking', 'ccs_handle_group_booking');
add_action('wp_ajax_nopriv_ccs_group_booking', 'ccs_handle_group_booking');
 ...
 * =========================================
 */
function ccs_handle_course_booking() {
    // Use controller if available, otherwise fall back to legacy implementation
    if (class_exists('\\CCS\\Controllers\\CourseBookingController')) {
        $controller = new \CCS\Controllers\CourseBookingController();
        $controller->handle();
        return;
    }
    ...
    try {
        ...
        // Comprehensive anti-bot validation
        $bot_check = ccs_validate_anti_bot('course-booking');
        if ($bot_check === false) {
            // Bot detected - silently accept but don't process
            wp_send_json_success([
    ...
    $saved = ccs_save_form_submission($submission_data, 'course-booking', false, '');
    ...
    if (!$sent) {
        $email_error = 'Email sending failed';
        error_log('CCS Course Booking: Failed to send email');
    }
    ...
    if (!is_wp_error($saved)) {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            if ($sent) {
        error_log('CCS Course Booking: Successfully sent email');
            } else {
                error_log('CCS Course Booking: Email failed but submission saved (ID: ' . $saved . ')');
            }
    }
    ...
    wp_send_json_success([
        ...
        error_log('CCS Course Booking: Exception - ' . $e->getMessage());
        wp_send_json_error([
            'message' => 'An error occurred while processing your request. Please try again or contact us directly.',
            'code' => 'server_error'
        ], 500);
...
add_action('wp_ajax_ccs_course_booking', 'ccs_handle_course_booking');
add_action('wp_ajax_nopriv_ccs_course_booking', 'ccs_handle_course_booking');
 ...
 * =========================================
 */
function ccs_handle_course_search() {
...
add_action('wp_ajax_ccs_course_search', 'ccs_handle_course_search');
add_action('wp_ajax_nopriv_ccs_course_search', 'ccs_handle_course_search');

/**
 * Get all courses for dropdown selection
 */
function ccs_handle_get_courses() {
    $args = [
        'post_type' => 'course',
        'posts_per_page' => -1,
        'orderby' => 'title',
...
add_action('wp_ajax_ccs_get_courses', 'ccs_handle_get_courses');
add_action('wp_ajax_nopriv_ccs_get_courses', 'ccs_handle_get_courses');
 ...
 * =========================================
 */
function ccs_handle_callback_request() {
    // Use controller if available, otherwise fall back to legacy implementation
    if (class_exists('\\CCS\\Controllers\\CallbackRequestController')) {
        $controller = new \CCS\Controllers\CallbackRequestController();
        $controller->handle();
        return;
    }
    ...
    try {
        ...
        // Comprehensive anti-bot validation
        $bot_check = ccs_validate_anti_bot('callback-request');
        if ($bot_check === false) {
            // Bot detected - silently accept but don't process
            wp_send_json_success([
        ...
        if (!empty($email)) {
            $email_validation = ccs_validate_email($email, false);
            if (!$email_validation['valid']) {
                $errors['email'] = $email_validation['error'];
            }
        }
        ...
        $saved = ccs_save_form_submission($submission_data, 'callback-request', false, '');
        ...
        if (!$sent) {
            $email_error = 'Email sending failed';
            error_log('CCS Callback Request: Failed to send email');
        }
        ...
        if (!is_wp_error($saved)) {
            // Log successful saves (ID only for privacy)
            error_log('CCS Callback Request: Submission saved (ID: ' . $saved . ')');
            
            if (defined('WP_DEBUG') && WP_DEBUG) {
                if ($sent) {
                    error_log('CCS Callback Request: Email successfully sent');
                } else {
                    error_log('CCS Callback Request: Email failed but submission saved (ID: ' . $saved . ')');
                }
        }
        ...
        wp_send_json_success([
            ...
            // Log save failures (error message only, no personal data)
            error_log('CCS Callback Request: FAILED to save submission - ' . $saved->get_error_message());
            // Only show error if submission couldn't be saved
            wp_send_json_error([
                'message' => 'Unable to process your request. Please try again or call us directly.',
                'code' => 'save_failed'
        ...
        error_log('CCS Callback Request Error: ' . $e->getMessage());
        wp_send_json_error([
            'message' => 'An error occurred while processing your request. Please try again or contact us directly.',
            'code' => 'server_error'
        ], 500);
...
add_action('wp_ajax_ccs_callback_request', 'ccs_handle_callback_request');
add_action('wp_ajax_nopriv_ccs_callback_request', 'ccs_handle_callback_request');

/**
 * Care assessment request – sends to CCS_EMAIL_OFFICE
 */
function ccs_handle_care_assessment_request() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'ccs_nonce')) {
    ...
    $care_for = sanitize_text_field($_POST['care_for'] ?? '');
    $when_start = sanitize_text_field($_POST['when_start'] ?? '');
    $message = sanitize_textarea_field($_POST['message'] ?? '');
    $consent = isset($_POST['consent']) && ($_POST['consent'] === 'true' || $_POST['consent'] === 'on');
    $errors = [];
    if (empty($name)) {
    ...
    $data = array_filter(compact('name', 'email', 'phone', 'care_type', 'care_for', 'when_start', 'message'));
    $saved = ccs_save_form_submission($data, 'care-assessment-request', false, '');
    $subject = 'Care Assessment Request – ' . $name;
    $body = "New care assessment request:\n\nName: {$name}\nEmail: {$email}\nPhone: {$phone}\n";
    if ($care_type) {
        $body .= "Care type: {$care_type}\n";
    }
    if ($care_for) {
        $body .= "Care for: {$care_for}\n";
    }
    if ($when_start) {
        $body .= "When to start: {$when_start}\n";
    }
    $body .= "\nMessage:\n" . $message . "\n\nSubmitted: " . current_time('mysql');
    $sent = defined('CCS_EMAIL_OFFICE') ? wp_mail(CCS_EMAIL_OFFICE, $subject, $body, ['Content-Type: text/plain; charset=UTF-8', 'Reply-To: ' . $name . ' <' . $email . '>']) : false;
    wp_send_json_success(['message' => 'Thank you. We will contact you within 24 hours.']);
}
add_action('wp_ajax_ccs_care_assessment_request', 'ccs_handle_care_assessment_request');
add_action('wp_ajax_nopriv_ccs_care_assessment_request', 'ccs_handle_care_assessment_request');

/**
 * Commissioning enquiry – sends to CCS_EMAIL_OFFICE
 */
function ccs_handle_commissioning_enquiry() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'ccs_nonce')) {
        wp_send_json_error(['message' => 'Security verification failed. Please refresh and try again.', 'code' => 'nonce_failed'], 403);
    }
    ...
    $data = array_filter(compact('organization', 'contact_name', 'title', 'email', 'phone', 'inquiry_type', 'message'));
    $data['name'] = $contact_name;
    ccs_save_form_submission($data, 'commissioning-enquiry', false, '');
    $subject = 'Commissioning Enquiry – ' . ($organization ?: $contact_name);
    $body = "New commissioning enquiry:\n\nOrganization: {$organization}\nContact: {$contact_name}\nTitle: {$title}\nEmail: {$email}\nPhone: {$phone}\nInquiry type: {$inquiry_type}\n\nMessage:\n{$message}\n\nSubmitted: " . current_time('mysql');
    if (defined('CCS_EMAIL_OFFICE')) {
        wp_mail(CCS_EMAIL_OFFICE, $subject, $body, ['Content-Type: text/plain; charset=UTF-8', 'Reply-To: ' . $contact_name . ' <' . $email . '>']);
    }
    wp_send_json_success(['message' => 'Thank you. Our commissioning team will contact you within 2 business days.']);
}
add_action('wp_ajax_ccs_commissioning_enquiry', 'ccs_handle_commissioning_enquiry');
...
add_action('wp_ajax_ccs_career_application', 'ccs_handle_career_application');
add_action('wp_ajax_nopriv_ccs_career_application', 'ccs_handle_career_application');
 ...
 * =========================================
 */
function ccs_ajax_validate_discount_code() {
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'ccs_nonce')) {
        wp_send_json_error([
            'message' => 'Security verification failed. Please refresh the page and try again.',
            'code' => 'nonce_failed',
    ...
    $validation = ccs_validate_discount_code($discount_code);
    wp_send_json_success($validation);
}
add_action('wp_ajax_validate_discount_code', 'ccs_ajax_validate_discount_code');
add_action('wp_ajax_nopriv_validate_discount_code', 'ccs_ajax_validate_discount_code');
...
function ccs_save_form_submission($data, $form_type, $email_sent = false, $email_error = '') {
    ...
    if (class_exists('\\CCS\\Repositories\\FormSubmissionRepository')) {
        $repository = new \CCS\Repositories\FormSubmissionRepository();
    ...
    if (is_wp_error($post_id)) {
        error_log('CCS Form Submission: Failed to save submission - ' . $post_id->get_error_message());
        error_log('CCS Form Submission: Post data: ' . print_r($post_data, true));
        return $post_id;
    }
    
    // Log successful creation (ID and type only for privacy)
    error_log('CCS Form Submission: Created submission post ID ' . $post_id . ' (Type: ' . $form_type . ')');
    
    // Add activity log entry for submission creation
    if (function_exists('ccs_add_activity_log')) {
        $form_type_label = $form_type ? ucfirst(str_replace('-', ' ', $form_type)) : 'Form';
        $submission_date = get_the_date('j M Y, g:i a', $post_id);
        ccs_add_activity_log($post_id, 'created', 'submission created', "Type: {$form_type_label} | Submitted: {$submission_date}");
    }
    ...
    if ($marketing_consent && !empty($email) && $form_type !== 'newsletter') {
        ...
        $newsletter_result = ccs_add_newsletter_subscriber($email, ccs_get_client_ip(), $first_name, $last_name);
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            ...
            error_log('CCS Form Submission: Newsletter subscription - ' . $status . ' from ' . $form_type . ' form');
        }
    } else {
    ...
    ccs_track_email_failures($email_sent);
 ...
 */
function ccs_track_email_failures($email_sent) {
    $failure_count_key = 'ccs_email_failure_count';
    $last_alert_key = 'ccs_email_failure_alert_sent';
    $failure_threshold = 3;
    ...
    if ($email_sent) {
        ...
        if ($new_count >= $failure_threshold) {
            ...
            if ($last_alert === false) {
                // Send alert email
                $alert_sent = ccs_send_email_failure_alert($new_count);
 ...
 */
function ccs_send_email_failure_alert($failure_count) {
    $to = CCS_EMAIL_OFFICE;
    $subject = '[URGENT] CCS Website: Multiple Form Submission Email Failures';
 ...
 */
function ccs_validate_uk_phone($phone) {
    if (empty($phone)) {
 ...
 */
function ccs_validate_name($name) {
    if (empty($name)) {
 ...
 * @return array ['valid' => bool, 'error' => string|null] Validation result
 */
function ccs_validate_email($email, $required = true) {
    if (empty($email)) {
        if ($required) {
 ...
 */
function ccs_validate_anti_bot($form_type = 'general') {
    $ip = ccs_get_client_ip();
    ...
    foreach ($honeypot_fields as $field) {
        if (!empty($_POST[$field])) {
            // Bot detected - silently accept but don't process
        if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("CCS Anti-Bot: Honeypot field '{$field}' filled by IP {$ip}");
            }
            return false;
        }
    }
    ...
    $recaptcha_secret = function_exists('ccs_get_recaptcha_secret_key') ? ccs_get_recaptcha_secret_key() : get_theme_mod('ccs_recaptcha_secret_key', '');
    ...
    if (!empty($recaptcha_secret)) {
        ...
        if (empty($recaptcha_response)) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("CCS Anti-Bot: No reCAPTCHA response from IP {$ip} - allowing submission");
            }
        ...
        if (is_wp_error($verify_response)) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("CCS Anti-Bot: reCAPTCHA verification network error: " . $verify_response->get_error_message());
            }
            ...
            if (!isset($verify_result['success']) || !$verify_result['success']) {
                ...
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    $error_msg = "CCS Anti-Bot: reCAPTCHA verification failed for IP {$ip} on domain {$domain}";
    ...
    if ($form_load_time > 0) {
        ...
        if ($time_taken < $minimum_time) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("CCS Anti-Bot: Form submitted extremely quickly ({$time_taken}s) by IP {$ip}");
            }
            return false;
        }
        ...
        if ($time_taken > $maximum_time) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("CCS Anti-Bot: Form submitted after too long ({$time_taken}s) by IP {$ip}");
            }
            return false;
        }
    }
    ...
    $rate_limit_key = 'ccs_form_submission_' . md5($ip . $form_type);
    ...
    if ($rate_limit_count >= $rate_limit_max) {
        // Rate limit exceeded
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("CCS Anti-Bot: Rate limit exceeded ({$rate_limit_count}/{$rate_limit_max}) by IP {$ip} for form {$form_type}");
        }
        return false;
    }
    ...
    if (empty($user_agent)) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("CCS Anti-Bot: Missing user agent from IP {$ip}");
        }
        return false;
    }
    ...
    foreach ($bot_patterns as $pattern) {
        if (strpos($user_agent_lower, $pattern) !== false) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("CCS Anti-Bot: Bot user agent detected ({$pattern}) from IP {$ip}");
            }
            return false;
        }
    }
    ...
    if (!empty($referrer)) {
        ...
        if ($referrer_host && $referrer_host !== $site_host) {
            ...
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log("CCS Anti-Bot: External referrer ({$referrer_host}) from IP {$ip}");
            }
        }
    }
    ...
    if (count($field_values) > 1 && count(array_unique($field_values)) === 1) {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log("CCS Anti-Bot: Suspicious field pattern (identical values) from IP {$ip}");
        }
        return false;
    }
