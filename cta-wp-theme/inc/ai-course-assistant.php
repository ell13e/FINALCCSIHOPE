 ...
 */
function cta_get_authoritative_sources_for_course($course_title, $category = '') {
 ...
 */
function cta_get_target_keywords_for_course($course_title, $category = '') {
    $title_lower = strtolower($course_title);
 ...
 */
function cta_get_course_specific_prompt_enhancements($course_title, $context) {
    $title_lower = strtolower($course_title);
    $enhancements = '';
 ...
 */
function cta_get_what_to_cover_for_course($course_title, $context) {
    $title_lower = strtolower($course_title);
    $what_to_cover = [];
 ...
 */
function cta_get_faq_template_for_course($course_title, $context) {
    $title_lower = strtolower($course_title);
    $faq_template = [];
 ...
 */
function cta_get_word_count_target($course_title) {
    $title_lower = strtolower($course_title);
 ...
 */
function cta_build_seo_prompt($course_title, $context, $content_type, $word_count = null) {
    $keywords = cta_get_target_keywords_for_course($course_title, $context['category'] ?? '');
    $sources = cta_get_authoritative_sources_for_course($course_title, $context['category'] ?? '');
    $course_specific = cta_get_course_specific_prompt_enhancements($course_title, $context);
 ...
 */
function cta_generate_course_intro_ajax() {
    check_ajax_referer('cta_ai_course', 'nonce');
...
Write only the paragraph text, nothing else. Do not include quotes or prefixes.";
    ...
    $intro = cta_call_ai_api_for_field('', $provider, $system_prompt, $prompt, 'wysiwyg');
...
add_action('wp_ajax_cta_generate_course_intro', 'cta_generate_course_intro_ajax');

/**
 * AJAX handler for generating "Why This Course Matters" content
 */
function cta_generate_course_why_matters_ajax() {
    check_ajax_referer('cta_ai_course', 'nonce');
    ...
    $context = cta_get_ai_context_for_field('course_why_matters', $post_id);
    $course_title = $context['title'];
    
    $system_prompt = "You are an expert SEO content writer for Continuity Training Academy, a care sector training provider in Kent, UK. You write compelling content explaining why training is important, focusing on CQC compliance, legal requirements, and professional development. You ONLY reference UK authoritative sources (CQC, HSE, NICE, Department of Health, Skills for Care, SCIE). Use British English only. Never make up statistics or claim compliance without verification. Write in a direct, conversational tone (not corporate).";
    
    $seo_prompt = cta_build_seo_prompt($course_title, $context, 'why_matters', '150-180 words');
    $what_to_cover = cta_get_what_to_cover_for_course($course_title, $context);
...
Write only the content, nothing else.";
    ...
    $content = cta_call_ai_api_for_field('', $provider, $system_prompt, $prompt, 'wysiwyg');
...
add_action('wp_ajax_cta_generate_course_why_matters', 'cta_generate_course_why_matters_ajax');

/**
 * AJAX handler for generating course FAQs
 */
function cta_generate_course_faqs_ajax() {
    check_ajax_referer('cta_ai_course', 'nonce');
    ...
    $context = cta_get_ai_context_for_field('course_faqs', $post_id);
    $course_title = $context['title'];
    
    $system_prompt = "You are an expert SEO content writer for Continuity Training Academy, a care sector training provider in Kent, UK. Write helpful, SEO-optimized FAQs that answer common questions about training courses. You ONLY reference UK authoritative sources (CQC, HSE, NICE, Department of Health, Skills for Care, SCIE). Use British English only. Never make up statistics or claim compliance without verification. Write in a direct, conversational tone (not corporate).";
    
    $keywords = cta_get_target_keywords_for_course($course_title, $context['category'] ?? '');
    $sources = cta_get_authoritative_sources_for_course($course_title, $context['category'] ?? '');
    $faq_template = cta_get_faq_template_for_course($course_title, $context);
    $course_specific = cta_get_course_specific_prompt_enhancements($course_title, $context);
    $what_to_cover = cta_get_what_to_cover_for_course($course_title, $context);
...
[
  ...
  {\"question\": \"Question text here\", \"answer\": \"Answer text here\"}
    ...
    $result = cta_call_ai_api_for_field('', $provider, $system_prompt, $prompt, 'textarea');
    ...
    jQuery(document).ready(function($) {
        ...
        $('#cta-generate-course-intro').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_generate_course_intro',
                    post_id: <?php echo $post->ID; ?>,
                    nonce: '<?php echo wp_create_nonce('cta_ai_course'); ?>'
                },
                success: function(response) {
        ...
        $('#cta-generate-course-why-matters').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_generate_course_why_matters',
                    post_id: <?php echo $post->ID; ?>,
                    nonce: '<?php echo wp_create_nonce('cta_ai_course'); ?>'
                },
                success: function(response) {
        ...
        $('#cta-generate-course-faqs').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_generate_course_faqs',
                    post_id: <?php echo $post->ID; ?>,
                    nonce: '<?php echo wp_create_nonce('cta_ai_course'); ?>'
                },
                success: function(response) {
