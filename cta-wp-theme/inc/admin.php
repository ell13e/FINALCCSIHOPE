 ...
 */
function cta_add_dashboard_widgets() {
    wp_add_dashboard_widget(
        'cta_upcoming_courses_widget',
        'Upcoming Courses',
        'cta_dashboard_widget_content'
    );
    ...
    if (!empty($ga_id)) {
        wp_add_dashboard_widget(
            'cta_google_analytics_widget',
            'Website Analytics',
            'cta_google_analytics_widget_content'
        );
    }
}
add_action('wp_dashboard_setup', 'cta_add_dashboard_widgets');

/**
 * Dashboard widget content
 */
function cta_dashboard_widget_content() {
 ...
 */
function cta_google_analytics_widget_content() {
    $ga_id = get_option('cta_google_analytics_id', '');
    ...
    $cache_key = 'cta_ga_stats_' . md5($ga_id);
    ...
    jQuery(document).ready(function($) {
        ...
        $('#cta_google_analytics_widget').on('click', '.cta-ga-refresh', function(e) {
            e.preventDefault();
 ...
 * Custom login logo
 */
function cta_login_logo() {
    $logo_url = CTA_THEME_URI . '/assets/img/logo/long_logo-400w.webp';
    ?>
    <style type="text/css">
...
add_action('login_enqueue_scripts', 'cta_login_logo');

/**
 * Custom login logo URL
 */
function cta_login_logo_url() {
    return home_url('/');
}
add_filter('login_headerurl', 'cta_login_logo_url');

/**
 * Custom login logo title
 */
function cta_login_logo_title() {
    return get_bloginfo('name');
}
add_filter('login_headertext', 'cta_login_logo_title');

/**
 * Add admin bar menu for quick access
 */
function cta_admin_bar_menu($wp_admin_bar) {
...
add_action('admin_bar_menu', 'cta_admin_bar_menu', 100);

/**
 * Admin footer text
 */
function cta_admin_footer_text($text) {
    return 'Continuity Training Academy Theme &bull; <a href="https://continuitytrainingacademy.co.uk" target="_blank">Visit Site</a>';
}
add_filter('admin_footer_text', 'cta_admin_footer_text');

/**
 * Remove unnecessary dashboard widgets
 */
function cta_remove_dashboard_widgets() {
    remove_meta_box('dashboard_quick_press', 'dashboard', 'side');
    remove_meta_box('dashboard_primary', 'dashboard', 'side');
}
add_action('wp_dashboard_setup', 'cta_remove_dashboard_widgets');

/**
 * Remove tags meta box from post editor
 */
function cta_remove_tags_meta_box() {
    remove_meta_box('tagsdiv-post_tag', 'post', 'side');
}
add_action('admin_menu', 'cta_remove_tags_meta_box');

/**
 * Auto-generate session title from course and date
 */
function cta_auto_generate_session_title($post_id, $post, $update) {
    ...
    remove_action('save_post', 'cta_auto_generate_session_title', 20, 3);
    ...
    add_action('save_post', 'cta_auto_generate_session_title', 20, 3);
}
add_action('save_post', 'cta_auto_generate_session_title', 20, 3);
 ...
 */
function cta_course_row_actions($actions, $post) {
...
add_filter('post_row_actions', 'cta_course_row_actions', 10, 2);

/**
 * Pre-fill course when creating session from course
 */
function cta_prefill_session_course() {
    global $pagenow;
...
add_action('admin_footer-post-new.php', 'cta_prefill_session_course');
...
add_action('wp_ajax_cta_get_course_title', 'cta_get_course_title_ajax');

/**
 * Ensure only one event can be featured at a time
 */
function cta_handle_featured_event($post_id) {
    // Check if ACF is available
...
add_action('acf/save_post', 'cta_handle_featured_event', 20);
 ...
 */
function cta_sync_featured_image_to_acf($post_id) {
...
add_action('save_post', 'cta_preserve_manual_featured_image', 20);
add_action('acf/save_post', 'cta_preserve_manual_featured_image', 20);
 ...
 */
function cta_auto_set_event_spaces($post_id) {
    // Check if ACF is available
    if (!function_exists('get_field') || !function_exists('update_field')) {
        return;
    }
    ...
    if (get_post_meta($post_id, '_cta_spaces_initialized', true)) {
        return;
    }
    ...
    update_post_meta($post_id, '_cta_spaces_initialized', true);
}
add_action('acf/save_post', 'cta_auto_set_event_spaces', 10);
 ...
 */
function cta_event_status_column_content($column, $post_id) {
...
add_action('manage_course_event_posts_custom_column', 'cta_event_status_column_content', 10, 2);

/**
 * Add Eventbrite sync status column to event list
 */
function cta_add_eventbrite_status_column($columns) {
    $columns['eventbrite_status'] = 'Eventbrite';
    return $columns;
}
add_filter('manage_course_event_posts_columns', 'cta_add_eventbrite_status_column');

function cta_eventbrite_status_column_content($column, $post_id) {
...
add_action('manage_course_event_posts_custom_column', 'cta_eventbrite_status_column_content', 10, 2);
 ...
 */
function cta_eventbrite_integration_status_notice($post) {
...
add_action('edit_form_after_title', 'cta_eventbrite_integration_status_notice');

/**
 * Add manual booking tracking section in event editor
 */
function cta_add_manual_bookings_field() {
    global $post;
        ...
        jQuery(document).ready(function($) {
            $('#update-wordpress-bookings').on('click', function() {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        action: 'cta_update_wordpress_bookings',
                        post_id: postId,
                        bookings: bookings,
                        nonce: '<?php echo wp_create_nonce('cta_update_bookings'); ?>'
                    },
                    success: function(response) {
                        if (response.success) {
                            location.reload();
...
add_action('post_submitbox_misc_actions', 'cta_add_manual_bookings_field');

/**
 * AJAX handler for manual booking adjustment
 */
function cta_ajax_update_wordpress_bookings() {
    check_ajax_referer('cta_update_bookings', 'nonce');
    ...
    if (function_exists('cta_recalculate_spaces_available')) {
        cta_recalculate_spaces_available($post_id);
    }
    
    wp_send_json_success(['message' => 'Bookings updated']);
}
add_action('wp_ajax_cta_update_wordpress_bookings', 'cta_ajax_update_wordpress_bookings');


/**
 * AJAX handler for inline field editing
 */
function cta_save_inline_field() {
    check_ajax_referer('cta_inline_edit', 'nonce');
...
add_action('wp_ajax_cta_save_inline_field', 'cta_save_inline_field');
...
add_action('admin_head-post.php', 'cta_improve_post_editor_ux');
...
function cta_enqueue_admin_media_library() {
    ...
    wp_enqueue_script(
        ...
        CTA_THEME_URI . '/assets/js/admin-media-library.js',
        ['jquery'],
        CTA_THEME_VERSION,
        true
    );
    ...
    wp_localize_script('cta-admin-media-library', 'ctaAdminMediaLibrary', [
        'nonce' => wp_create_nonce('cta_save_image'),
    ]);
    ...
    wp_add_inline_style('wp-admin', '
        ...
        .wp-list-table .column-cta_thumbnail,
...
function cta_save_image_ajax() {
    check_ajax_referer('cta_save_image', 'nonce');
