 ...
 */
function cta_ai_supported_providers() {
 ...
 */
function cta_ai_provider_fallback_order($preferred = 'groq') {
    $preferred = is_string($preferred) ? strtolower(trim($preferred)) : 'groq';
    if (!in_array($preferred, cta_ai_supported_providers(), true)) {
        $preferred = 'groq';
    }
    ...
    foreach (cta_ai_supported_providers() as $p) {
 ...
 */
function cta_ai_provider_key_option($provider) {
    $provider = is_string($provider) ? strtolower(trim($provider)) : '';
    if (!in_array($provider, cta_ai_supported_providers(), true)) {
        return null;
    }
    return 'cta_ai_' . $provider . '_api_key';
}

/**
 * Get API key for a provider.
 * - Prefer provider-specific option (cta_ai_groq_api_key, etc.)
 * - Fall back to legacy global option (cta_ai_api_key)
 */
function cta_ai_get_api_key_for_provider($provider) {
    $opt = cta_ai_provider_key_option($provider);
    $specific = $opt ? trim((string) get_option($opt, '')) : '';
    if (!empty($specific)) {
        return $specific;
    }
    return trim((string) get_option('cta_ai_api_key', ''));
}

/**
 * Validate that a key "looks like" it belongs to a provider.
 */
function cta_ai_key_looks_valid($provider, $api_key) {
    $provider = is_string($provider) ? strtolower(trim($provider)) : '';
 ...
 */
function cta_ai_get_attemptable_providers($preferred = 'groq') {
    $providers = cta_ai_provider_fallback_order($preferred);
    $out = [];
    foreach ($providers as $p) {
        $key = cta_ai_get_api_key_for_provider($p);
        if (cta_ai_key_looks_valid($p, $key)) {
            $out[] = $p;
        }
    }
    return $out;
 ...
 */
function cta_ai_try_providers($preferred, callable $fn) {
    $providers = cta_ai_get_attemptable_providers($preferred);
    if (empty($providers)) {
        return new WP_Error(
            'ai_not_configured',
