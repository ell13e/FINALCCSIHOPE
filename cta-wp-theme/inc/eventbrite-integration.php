 ...
 */
function cta_eventbrite_api_request($url, $args = [], $operation_name = 'API Request') {
    $default_args = [
        'method' => 'GET',
        'headers' => [],
        'body' => null,
 ...
 * 
 * Follows Eventbrite API best practices via cta_eventbrite_api_request helper.
 * 
 ...
 */
function cta_create_eventbrite_discount($code, $percent_off = 20.0, $expiry_date = '', $event_id = null) {
    $org_id = get_option('cta_eventbrite_organization_id', '');
    ...
    return cta_eventbrite_api_request($url, [
        'method' => 'POST',
 ...
 */
function cta_update_eventbrite_discount($discount_id, $code = '', $percent_off = 20.0, $expiry_date = '') {
 ...
 */
function cta_delete_eventbrite_discount($discount_id) {
    if (empty($discount_id)) {
        return new WP_Error('invalid_discount_id', 'Discount ID cannot be empty');
    }
    ...
    $result = cta_eventbrite_api_request($url, [
        'method' => 'DELETE',
    ], 'Delete Discount');
    ...
    if ($error_code === 'DISCOUNT_CANNOT_BE_DELETED') {
        ...
        $update_result = cta_update_eventbrite_discount($discount_id, '', 20.0, date('Y-m-d', strtotime('-1 day')));
 ...
 */
function cta_sync_discount_code_to_eventbrite($code_data, $index) {
    ...
    if (!$sync_to_eventbrite) {
        if (!empty($eventbrite_discount_id)) {
            $delete_result = cta_delete_eventbrite_discount($eventbrite_discount_id);
    ...
    if (!$active) {
        if (!empty($eventbrite_discount_id)) {
            ...
            $result = cta_update_eventbrite_discount($eventbrite_discount_id, $code, $percent_off, $yesterday);
            if (!is_wp_error($result)) {
    ...
    if (!empty($eventbrite_discount_id)) {
        $result = cta_update_eventbrite_discount($eventbrite_discount_id, $code, $percent_off, $expiry_date);
        if (is_wp_error($result)) {
        ...
        $result = cta_create_eventbrite_discount($code, $percent_off, $expiry_date);
 ...
 */
function cta_upload_event_to_eventbrite($post_id) {
    $auto_upload = get_option('cta_eventbrite_auto_upload', 1);
    if (!$auto_upload) {
        return;
    }
    ...
    if (get_post_meta($post_id, '_cta_eventbrite_syncing', true)) {
        return;
    }
    ...
    update_post_meta($post_id, '_cta_eventbrite_syncing', true);
    
    $oauth_token = get_option('cta_eventbrite_oauth_token', '');
    $org_id = get_option('cta_eventbrite_organization_id', '');
    
    if (empty($oauth_token) || empty($org_id)) {
        delete_post_meta($post_id, '_cta_eventbrite_syncing');
        return;
    }
    
    $eventbrite_id = get_post_meta($post_id, 'eventbrite_id', true);
    
    $event_data = cta_prepare_eventbrite_event_data($post_id);
    ...
    if ($eventbrite_id) {
        $was_update = true;
        update_post_meta($post_id, '_cta_eventbrite_was_update', true);
        
        $result = cta_update_eventbrite_event($eventbrite_id, $event_data, $oauth_token);
        ...
        if (is_wp_error($result)) {
            ...
            if ($error_code === 'NOT_FOUND' || $http_code === 404) {
                ...
                delete_post_meta($post_id, '_cta_eventbrite_was_update');
                ...
                $error_message = cta_eventbrite_error_message($error_code, $result->get_error_message());
                set_transient('cta_eventbrite_upload_' . $post_id, [
                    'success' => false,
            ...
            if (isset($event_data['ticket_class'])) {
                cta_update_or_create_ticket_class($eventbrite_id, $event_data['ticket_class'], $oauth_token);
            }
            
            // Sync FAQs via Structured Content API
            if (!empty($event_data['faqs']) && is_array($event_data['faqs'])) {
                cta_sync_eventbrite_faqs_structured_content($eventbrite_id, $event_data['faqs'], $oauth_token);
            }
    ...
    if (!$eventbrite_id) {
        $result = cta_create_eventbrite_event($event_data, $oauth_token, $org_id);
        ...
        if (!is_wp_error($result) && isset($result['id'])) {
            ...
            if (isset($event_data['ticket_class'])) {
                cta_create_eventbrite_ticket_class($new_event_id, $event_data['ticket_class'], $oauth_token);
            }
            
            // Sync FAQs via Structured Content API
            if (!empty($event_data['faqs']) && is_array($event_data['faqs'])) {
                cta_sync_eventbrite_faqs_structured_content($new_event_id, $event_data['faqs'], $oauth_token);
            }
            
            $auto_publish = get_option('cta_eventbrite_auto_publish', 1);
            if ($auto_publish) {
                $publish_result = cta_publish_eventbrite_event($new_event_id, $oauth_token);
    ...
    if (isset($result) && is_wp_error($result)) {
        ...
        $was_update = get_post_meta($post_id, '_cta_eventbrite_was_update', true);
        delete_post_meta($post_id, '_cta_eventbrite_was_update');
 ...
 */
function cta_prepare_eventbrite_event_data($post_id) {
    $course = get_field('linked_course', $post_id);
    $event_date = get_field('event_date', $post_id);
    ...
    if ($eventbrite_custom_name) {
        ...
        $event_name = cta_generate_eventbrite_event_name($post_id);
    }
    
    if (empty($eventbrite_description)) {
        $eventbrite_description = cta_generate_eventbrite_description($post_id);
        if (empty($eventbrite_description)) {
    ...
    if (empty($eventbrite_summary)) {
        $eventbrite_summary = cta_generate_eventbrite_summary($post_id);
    }
    ...
    $venue_id = cta_get_or_create_eventbrite_venue($location);
    $tags = cta_generate_eventbrite_tags($post_id);
 ...
 */
function cta_generate_eventbrite_event_name($post_id) {
    $course = get_field('linked_course', $post_id);
    if (!$course) {
        return '';
    }
 ...
 */
function cta_generate_eventbrite_summary($post_id) {
    $course = get_field('linked_course', $post_id);
    if (!$course) {
        return '';
    }
 ...
 */
function cta_generate_eventbrite_tags($post_id) {
    $course = get_field('linked_course', $post_id);
    if (!$course) {
        return [];
    }
 ...
 */
function cta_generate_eventbrite_description($post_id) {
    ...
    $course_title = $course->post_title;
    $course_description = get_field('course_description', $course->ID);
    $outcomes = function_exists('cta_get_outcomes') ? cta_get_outcomes($course->ID) : get_field('course_outcomes', $course->ID);
    $accreditation = get_field('course_accreditation', $course->ID);
...
Always format responses as clean, semantic HTML ready for Eventbrite's platform.";
    ...
    $result = cta_ai_try_providers($preferred_provider, function($provider, $api_key) use ($system_prompt, $prompt) {
        if ($provider === 'groq' && function_exists('cta_call_groq_api')) {
            return cta_call_groq_api($api_key, $system_prompt, $prompt);
        }
        if ($provider === 'anthropic' && function_exists('cta_call_anthropic_api')) {
            return cta_call_anthropic_api($api_key, $system_prompt, $prompt);
        }
        if ($provider === 'openai' && function_exists('cta_call_openai_api')) {
            return cta_call_openai_api($api_key, $system_prompt, $prompt);
        }
 ...
 */
function cta_auto_generate_eventbrite_description($post_id) {
    ...
    // Generate with AI
    $description = cta_generate_eventbrite_description($post_id);
    if ($description) {
    ...
    if (empty($current_summary)) {
        $summary = cta_generate_eventbrite_summary($post_id);
        if ($summary) {
            update_field('eventbrite_summary', $summary, $post_id);
        }
    }
}
add_action('acf/save_post', 'cta_auto_generate_eventbrite_description', 15);

/**
 * Get or create Eventbrite venue
 * Defaults to "The Maidstone Studios" unless location field specifies otherwise
 */
function cta_get_or_create_eventbrite_venue($location = '') {
    $oauth_token = get_option('cta_eventbrite_oauth_token', '');
    $org_id = get_option('cta_eventbrite_organization_id', '');
    ...
    $venue_cache_key = 'cta_eventbrite_venue_' . md5($venue_name);
    ...
    $default_venue_id = get_option('cta_eventbrite_venue_id', '');
    ...
    $venue_id = cta_search_eventbrite_venue($venue_name, $org_id, $oauth_token);
    ...
    $venue_id = cta_create_eventbrite_venue($venue_data, $org_id, $oauth_token);
    ...
    if ($venue_id) {
        ...
        if ($venue_name === $default_venue_name && !$default_venue_id) {
            update_option('cta_eventbrite_venue_id', $venue_id);
        }
 ...
 * Follows Eventbrite API best practices via cta_eventbrite_api_request helper.
 */
function cta_search_eventbrite_venue($venue_name, $org_id, $oauth_token) {
    ...
    $data = cta_eventbrite_api_request($url, [
        'method' => 'GET',
        'timeout' => 15,
        'headers' => [
 ...
 * Follows Eventbrite API best practices via cta_eventbrite_api_request helper.
 */
function cta_create_eventbrite_venue($venue_data, $org_id, $oauth_token) {
    if (empty($org_id)) {
 ...
 * Follows Eventbrite API best practices via cta_eventbrite_api_request helper.
 */
function cta_update_eventbrite_event($eventbrite_id, $event_data, $oauth_token) {
 ...
 * Follows Eventbrite API best practices via cta_eventbrite_api_request helper.
 */
function cta_publish_eventbrite_event($event_id, $oauth_token) {
    if (empty($event_id)) {
        return new WP_Error('missing_event_id', 'Eventbrite event ID is required');
    }
 ...
 */
function cta_sync_eventbrite_faqs_structured_content($eventbrite_id, $faqs, $oauth_token) {
    if (empty($eventbrite_id) || empty($faqs) || !is_array($faqs)) {
        return false;
    }
    
    // Get current working version of structured content
    $current_content = cta_get_eventbrite_structured_content($eventbrite_id, $oauth_token);
    $current_version = 1;
    $existing_modules = [];
    $existing_widgets = [];
 ...
 */
function cta_get_eventbrite_structured_content($eventbrite_id, $oauth_token) {
    if (empty($eventbrite_id)) {
        return new WP_Error('missing_event_id', 'Eventbrite event ID is required');
    }
 ...
 * Follows Eventbrite API best practices via cta_eventbrite_api_request helper.
 */
function cta_create_eventbrite_ticket_class($event_id, $ticket_class_data, $oauth_token) {
    if (empty($event_id)) {
 ...
 * Follows Eventbrite API best practices via cta_eventbrite_api_request helper.
 */
function cta_update_or_create_ticket_class($event_id, $ticket_class_data, $oauth_token) {
    if (empty($event_id)) {
        return false;
    }
    ...
    if (!is_wp_error($data) && isset($data['ticket_classes']) && !empty($data['ticket_classes'])) {
        ...
        $update_result = cta_eventbrite_api_request($update_url, [
            'method' => 'POST',
            'body' => json_encode($ticket_class_data),
            'headers' => [
                'Authorization' => 'Bearer ' . $oauth_token,
    ...
    $create_result = cta_create_eventbrite_ticket_class($event_id, $ticket_class_data, $oauth_token);
 ...
 */
function cta_save_event_to_eventbrite($post_id) {
    ...
    wp_schedule_single_event(time() + 2, 'cta_upload_eventbrite_delayed', [$post_id]);
}
add_action('save_post', 'cta_save_event_to_eventbrite', 20);

/**
 * Delayed upload to ensure ACF fields are saved
 */
function cta_upload_eventbrite_delayed($post_id) {
    cta_upload_event_to_eventbrite($post_id);
}
add_action('cta_upload_eventbrite_delayed', 'cta_upload_eventbrite_delayed');

/**
 * Also hook into ACF save for immediate updates
 */
function cta_acf_save_event_to_eventbrite($post_id) {
    if (get_post_type($post_id) === 'course_event') {
        // Small delay to batch multiple field updates
        wp_clear_scheduled_hook('cta_upload_eventbrite_delayed', [$post_id]);
        wp_schedule_single_event(time() + 1, 'cta_upload_eventbrite_delayed', [$post_id]);
    }
}
add_action('acf/save_post', 'cta_acf_save_event_to_eventbrite', 20);

/**
 * Track WordPress booking when form is submitted
 */
function cta_track_wordpress_booking($post_id, $delegates = 1) {
 ...
 */
function cta_recalculate_spaces_available($post_id) {
    if (!function_exists('get_field') || !function_exists('update_field')) {
        return;
    }
 ...
 */
function cta_auto_sync_spaces_from_eventbrite() {
    $auto_sync = get_option('cta_eventbrite_auto_sync_spaces', 1);
    if (!$auto_sync) {
        return;
    }
    ...
    foreach ($events as $event) {
        $result = cta_sync_spaces_from_eventbrite($event->ID);
        if ($result) {
            $synced++;
        } else {
            $errors++;
    ...
    // Log sync results
    update_option('cta_eventbrite_last_sync', [
 ...
 */
function cta_schedule_eventbrite_auto_sync() {
    // Clear existing schedule
    wp_clear_scheduled_hook('cta_eventbrite_auto_sync_spaces');
    
    // Get sync frequency (default: hourly)
    $frequency = get_option('cta_eventbrite_sync_frequency', 'hourly');
    
    // Schedule if auto-sync is enabled
    $auto_sync = get_option('cta_eventbrite_auto_sync_spaces', 1);
    if ($auto_sync) {
        if (!wp_next_scheduled('cta_eventbrite_auto_sync_spaces')) {
            wp_schedule_event(time(), $frequency, 'cta_eventbrite_auto_sync_spaces');
        }
    }
}
add_action('init', 'cta_schedule_eventbrite_auto_sync');

/**
 * Hook the scheduled sync
 */
add_action('cta_eventbrite_auto_sync_spaces', 'cta_auto_sync_spaces_from_eventbrite');

/**
 * Re-schedule when settings change
 */
function cta_reschedule_eventbrite_sync() {
    cta_schedule_eventbrite_auto_sync();
}
add_action('update_option_cta_eventbrite_auto_sync_spaces', 'cta_reschedule_eventbrite_sync');
add_action('update_option_cta_eventbrite_sync_frequency', 'cta_reschedule_eventbrite_sync');

/**
 * Add Eventbrite import section to course_event editor
 */
function cta_add_eventbrite_import_section() {
    global $post;
    
    if ($post && $post->post_type === 'course_event') {
        add_meta_box(
            'cta_eventbrite_import',
            'Import from Eventbrite',
            'cta_eventbrite_import_meta_box',
            'course_event',
            'normal',
            'high'
        );
...
add_action('add_meta_boxes', 'cta_add_eventbrite_import_section');

/**
 * Display admin notices for Eventbrite operations
 */
function cta_eventbrite_admin_notices() {
    ...
    $upload_status = get_transient('cta_eventbrite_upload_' . $post_id);
    if ($upload_status) {
        ...
        delete_transient('cta_eventbrite_upload_' . $post_id);
    }
}
add_action('admin_notices', 'cta_eventbrite_admin_notices');

/**
 * Enhanced error messages with actionable tips
 */
function cta_eventbrite_error_message($error_code, $error_message) {
    $help = '';
 ...
 */
function cta_eventbrite_status_indicator($post_id) {
    $auto_upload = get_option('cta_eventbrite_auto_upload', 1);
    $eventbrite_id = get_post_meta($post_id, 'eventbrite_id', true);
 ...
 */
function cta_eventbrite_quick_start_guide($post_id) {
    $eventbrite_id = get_post_meta($post_id, 'eventbrite_id', true);
 ...
 */
function cta_eventbrite_validation_warnings($post_id) {
    $warnings = [];
 ...
 */
function cta_eventbrite_test_connection() {
    check_ajax_referer('cta_test_eventbrite', 'nonce');
    ...
    if (is_wp_error($data)) {
        ...
        $error_code = isset($error_data['error_code']) ? $error_data['error_code'] : '';
        $error_message = cta_eventbrite_error_message($error_code, $data->get_error_message());
...
add_action('wp_ajax_cta_test_eventbrite_connection', 'cta_eventbrite_test_connection');

/**
 * Eventbrite import meta box content
 */
function cta_eventbrite_import_meta_box($post) {
    $oauth_token = get_option('cta_eventbrite_oauth_token', '');
    $org_id = get_option('cta_eventbrite_organization_id', '');
    ...
    <div id="cta-eventbrite-import-wrapper" style="padding: 15px;">
        ...
        cta_eventbrite_quick_start_guide($post->ID);
        
        // Validation Warnings
        cta_eventbrite_validation_warnings($post->ID);
        
        // Status Indicator
        cta_eventbrite_status_indicator($post->ID);
        ?>
    ...
    jQuery(document).ready(function($) {
        ...
        $('#cta-fetch-eventbrite-events').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_fetch_eventbrite_events',
                    nonce: '<?php echo wp_create_nonce('cta_fetch_events'); ?>'
                },
        ...
        $('#cta-confirm-import').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_import_eventbrite_event',
                    post_id: <?php echo $post->ID; ?>,
                    eventbrite_event: JSON.stringify(selectedEvent),
                    nonce: '<?php echo wp_create_nonce('cta_import_event'); ?>'
                },
                success: function(response) {
                    if (response.success) {
        ...
        $('#cta-unlink-eventbrite').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_unlink_eventbrite',
                    post_id: <?php echo $post->ID; ?>,
                    nonce: '<?php echo wp_create_nonce('cta_unlink_event'); ?>'
                },
                success: function(response) {
                    if (response.success) {
                        location.reload();
 ...
 */
function cta_ajax_fetch_eventbrite_events() {
    check_ajax_referer('cta_fetch_events', 'nonce');
...
add_action('wp_ajax_cta_fetch_eventbrite_events', 'cta_ajax_fetch_eventbrite_events');

/**
 * AJAX: Import Eventbrite event and fill fields
 */
function cta_ajax_import_eventbrite_event() {
    check_ajax_referer('cta_import_event', 'nonce');
    ...
    $course_id = cta_find_matching_course($event_name);
    
    // Get ticket class for price
    $price = 0;
    if ($eventbrite_id) {
        $price = cta_get_eventbrite_ticket_price($eventbrite_id);
    }
...
add_action('wp_ajax_cta_import_eventbrite_event', 'cta_ajax_import_eventbrite_event');

/**
 * Find matching course by title
 */
function cta_find_matching_course($event_name) {
 ...
 * Follows Eventbrite API best practices via cta_eventbrite_api_request helper.
 */
function cta_get_eventbrite_ticket_price($event_id) {
    if (empty($event_id)) {
        return 0;
    }
 ...
 */
function cta_ajax_unlink_eventbrite() {
    check_ajax_referer('cta_unlink_event', 'nonce');
...
add_action('wp_ajax_cta_unlink_eventbrite', 'cta_ajax_unlink_eventbrite');

/**
 * AJAX handler for regenerate button
 */
function cta_ajax_regenerate_eventbrite_description() {
    check_ajax_referer('cta_regenerate_desc', 'nonce');
...
add_action('wp_ajax_cta_regenerate_eventbrite_description', 'cta_ajax_regenerate_eventbrite_description');

/**
 * AJAX handler for generating Eventbrite Summary (140 chars)
 */
function cta_generate_eventbrite_summary_ajax() {
    check_ajax_referer('cta_regenerate_desc', 'nonce');
    ...
    $system_prompt = "You are an expert copywriter for Continuity Training Academy, a care sector training provider in Kent, UK. Write concise, compelling summaries optimized for Eventbrite search and discovery. Use British English only.";
...
Write only the summary text, nothing else. No quotes, no prefixes, just the 140-character summary.";
    ...
    $result = cta_ai_try_providers($preferred_provider, function($provider, $api_key) use ($system_prompt, $prompt) {
        if ($provider === 'groq' && function_exists('cta_call_groq_api')) return cta_call_groq_api($api_key, $system_prompt, $prompt);
        if ($provider === 'anthropic' && function_exists('cta_call_anthropic_api')) return cta_call_anthropic_api($api_key, $system_prompt, $prompt);
        if ($provider === 'openai' && function_exists('cta_call_openai_api')) return cta_call_openai_api($api_key, $system_prompt, $prompt);
        return new WP_Error('api_error', 'Provider not available');
    });
...
add_action('wp_ajax_cta_generate_eventbrite_summary', 'cta_generate_eventbrite_summary_ajax');

/**
 * AJAX handler for generating Eventbrite Custom Name (75 chars)
 */
function cta_generate_eventbrite_custom_name_ajax() {
    check_ajax_referer('cta_regenerate_desc', 'nonce');
    ...
    // Build context-aware prompt
    $system_prompt = "You are an expert SEO copywriter for Continuity Training Academy, a care sector training provider in Kent, UK. Write SEO-optimized event names for Eventbrite that maximize search visibility. Use British English only.";
...
add_action('wp_ajax_cta_generate_eventbrite_custom_name', 'cta_generate_eventbrite_custom_name_ajax');

/**
 * AJAX handler for generating Eventbrite FAQs
 */
function cta_generate_eventbrite_faqs_ajax() {
    check_ajax_referer('cta_regenerate_desc', 'nonce');
    ...
    // Build context-aware prompt
    $system_prompt = "You are an expert copywriter for Continuity Training Academy, a care sector training provider in Kent, UK. Generate FAQ questions and answers optimized for Eventbrite listings to improve search visibility (+8% boost). Use British English only.";
...
add_action('wp_ajax_cta_generate_eventbrite_faqs', 'cta_generate_eventbrite_faqs_ajax');

/**
 * AJAX handler for generating Eventbrite Tag Suggestions (AI-powered)
 */
function cta_generate_eventbrite_tags_ai_ajax() {
    check_ajax_referer('cta_regenerate_desc', 'nonce');
    ...
    // Build context-aware prompt
    $system_prompt = "You are an expert SEO copywriter for Continuity Training Academy, a care sector training provider in Kent, UK. Generate Eventbrite tag suggestions optimized for search visibility and discovery. Use British English only.";
...
add_action('wp_ajax_cta_generate_eventbrite_tags_ai', 'cta_generate_eventbrite_tags_ai_ajax');

/**
 * Add JavaScript for regenerate button
 */
function cta_eventbrite_description_admin_script($hook) {
    global $post;
    ...
    jQuery(document).ready(function($) {
        ...
        $('#cta-regenerate-eventbrite-desc').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_regenerate_eventbrite_description',
                    post_id: <?php echo $post->ID; ?>,
                    nonce: '<?php echo wp_create_nonce('cta_regenerate_desc'); ?>'
                },
                success: function(response) {
                    if (response.success) {
        ...
        $('#cta-generate-eventbrite-summary').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_generate_eventbrite_summary',
                    post_id: <?php echo $post->ID; ?>,
                    nonce: '<?php echo wp_create_nonce('cta_regenerate_desc'); ?>'
                },
                success: function(response) {
        ...
        $('#cta-generate-eventbrite-name').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_generate_eventbrite_custom_name',
                    post_id: <?php echo $post->ID; ?>,
                    nonce: '<?php echo wp_create_nonce('cta_regenerate_desc'); ?>'
                },
                success: function(response) {
        ...
        $('#cta-generate-eventbrite-faqs').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_generate_eventbrite_faqs',
                    post_id: <?php echo $post->ID; ?>,
                    nonce: '<?php echo wp_create_nonce('cta_regenerate_desc'); ?>'
                },
                success: function(response) {
        ...
        $('#cta-generate-eventbrite-tags').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_generate_eventbrite_tags_ai',
                    post_id: <?php echo $post->ID; ?>,
                    nonce: '<?php echo wp_create_nonce('cta_regenerate_desc'); ?>'
                },
                success: function(response) {
...
add_action('admin_footer', 'cta_eventbrite_description_admin_script');
