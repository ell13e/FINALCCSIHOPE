 ...
 */
function cta_event_management_enqueue_assets($hook) {
    global $typenow;
...
add_action('admin_enqueue_scripts', 'cta_event_management_enqueue_assets');

/**
 * Override the default list table with modern card view
 */
function cta_event_management_override_list_table() {
    global $typenow;
    ...
    if ($typenow === 'course_event') {
        ...
        $active_count = cta_count_events_by_status('active');
        $draft_count = cta_count_events_by_status('draft');
        $past_count = cta_count_events_by_status('past');
 ...
 * Count events by status
 */
function cta_count_events_by_status($status) {
    $args = [
        'post_type' => 'course_event',
        'posts_per_page' => -1,
        'fields' => 'ids',
 ...
 */
function cta_event_management_show_card_view() {
    ...
    if ($typenow === 'course_event' && $pagenow === 'edit.php') {
        ...
        add_action('admin_notices', function() {
            cta_event_management_override_list_table();
        }, 1);
    }
}
add_action('load-edit.php', 'cta_event_management_show_card_view');
