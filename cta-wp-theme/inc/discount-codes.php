 ...
 */
function cta_discount_codes_menu() {
    ...
    add_submenu_page(
        ...
        $cap,
        'cta-discount-codes',
        'cta_discount_codes_page'
    );
}
add_action('admin_menu', 'cta_discount_codes_menu', 26);

/**
 * Get all discount codes
 */
function cta_get_discount_codes() {
    $codes = get_option('cta_discount_codes', []);
 ...
 * Save discount codes
 */
function cta_save_discount_codes($codes) {
    update_option('cta_discount_codes', $codes);
}
 ...
 */
function cta_validate_discount_code($code) {
    if (empty($code)) {
        return [
            'valid' => false,
            'message' => '',
 ...
 */
function cta_get_site_wide_discount() {
    return [
        'active' => get_option('cta_site_wide_discount_active', false),
        'percentage' => floatval(get_option('cta_site_wide_discount_percentage', 0)),
        'label' => get_option('cta_site_wide_discount_label', 'Site-Wide Sale'),
        'expiry_date' => get_option('cta_site_wide_discount_expiry', ''),
    ];
}
 ...
 */
function cta_is_site_wide_discount_active() {
    $discount = cta_get_site_wide_discount();
 ...
 */
function cta_get_site_wide_discount_percentage() {
    if (!cta_is_site_wide_discount_active()) {
        return 0;
    }
    $discount = cta_get_site_wide_discount();
 ...
 */
function cta_apply_site_wide_discount($original_price) {
    $discount_percent = cta_get_site_wide_discount_percentage();
 ...
 */
function cta_get_course_discount($course_id) {
    return [
 ...
 */
function cta_is_course_discount_active($course_id) {
    $discount = cta_get_course_discount($course_id);
 ...
 */
function cta_get_course_discount_percentage($course_id) {
    if (!cta_is_course_discount_active($course_id)) {
        return 0;
    }
    $discount = cta_get_course_discount($course_id);
    return $discount['percentage'];
 ...
 */
function cta_apply_course_discount($course_id, $original_price) {
    $discount_percent = cta_get_course_discount_percentage($course_id);
    if ($discount_percent <= 0) {
        return $original_price;
    }
    return $original_price * (1 - ($discount_percent / 100));
 ...
 */
function cta_get_course_discount_ajax() {
    check_ajax_referer('cta_discount_codes_nonce', 'nonce');
...
add_action('wp_ajax_cta_get_course_discount', 'cta_get_course_discount_ajax');

/**
 * Discount Codes admin page
 */
function cta_discount_codes_page() {
    $cap = function_exists('cta_newsletter_required_capability') ? cta_newsletter_required_capability() : 'edit_others_posts';
    if (!current_user_can($cap)) {
    ...
    if (isset($_POST['cta_save_site_wide_discount']) && check_admin_referer('cta_site_wide_discount_action')) {
        ...
        if ($percentage < 0 || $percentage > 100) {
            ...
            update_option('cta_site_wide_discount_active', $active);
            update_option('cta_site_wide_discount_percentage', $percentage);
            update_option('cta_site_wide_discount_label', $label);
    ...
    if (isset($_POST['cta_save_course_discount']) && check_admin_referer('cta_course_discount_action')) {
    ...
    if (isset($_POST['cta_add_discount_code']) && check_admin_referer('cta_discount_codes_action')) {
        ...
        if (!empty($code)) {
            ...
            if (!$exists) {
                ...
                if ($sync_to_eventbrite && $active && function_exists('cta_sync_discount_code_to_eventbrite')) {
                    // Add small delay to avoid rate limits (best practice)
                    usleep(200000); // 0.2 seconds delay
                    $new_code = cta_sync_discount_code_to_eventbrite($new_code, count($codes));
                }
                
                $codes[] = $new_code;
                cta_save_discount_codes($codes);
    ...
    // Handle delete
    if (isset($_GET['delete']) && check_admin_referer('cta_delete_discount_code_' . $_GET['delete'])) {
        ...
        if (isset($codes[$delete_index])) {
            ...
            $eventbrite_message = '';
            if (!empty($code_to_delete['eventbrite_discount_id']) && function_exists('cta_delete_eventbrite_discount')) {
                $delete_result = cta_delete_eventbrite_discount($code_to_delete['eventbrite_discount_id']);
    ...
    if (isset($_GET['toggle']) && check_admin_referer('cta_toggle_discount_code_' . $_GET['toggle'])) {
        ...
        if (isset($codes[$toggle_index])) {
            ...
            // Sync to Eventbrite if enabled (with rate limiting - best practice)
            if (!empty($codes[$toggle_index]['sync_to_eventbrite']) && function_exists('cta_sync_discount_code_to_eventbrite')) {
                // Add small delay to avoid rate limits (best practice)
                usleep(200000); // 0.2 seconds delay
                $codes[$toggle_index] = cta_sync_discount_code_to_eventbrite($codes[$toggle_index], $toggle_index);
            }
    ...
    if (isset($_GET['toggle_eventbrite']) && check_admin_referer('cta_toggle_eventbrite_sync_' . $_GET['toggle_eventbrite'])) {
        ...
        if (isset($codes[$toggle_index])) {
            ...
            if (!empty($codes[$toggle_index]['sync_to_eventbrite']) && function_exists('cta_sync_discount_code_to_eventbrite')) {
                $codes[$toggle_index] = cta_sync_discount_code_to_eventbrite($codes[$toggle_index], $toggle_index);
            } elseif (empty($codes[$toggle_index]['sync_to_eventbrite']) && !empty($codes[$toggle_index]['eventbrite_discount_id']) && function_exists('cta_delete_eventbrite_discount')) {
                // Add small delay to avoid rate limits (best practice)
                usleep(200000); // 0.2 seconds delay
                
                // Try to delete from Eventbrite if sync disabled
                $delete_result = cta_delete_eventbrite_discount($codes[$toggle_index]['eventbrite_discount_id']);
    ...
    if (isset($_GET['delete_course_discount']) && check_admin_referer('cta_delete_course_discount_' . $_GET['delete_course_discount'])) {
    ...
    if (isset($_GET['disable_course_discount']) && check_admin_referer('cta_disable_course_discount_' . $_GET['disable_course_discount'])) {
    ...
    $is_active = cta_is_site_wide_discount_active();
    ...
    foreach ($all_courses as $course) {
        if (cta_is_course_discount_active($course->ID)) {
    ...
    <div class="wrap">
        ...
        <div style="background: #fff; padding: 20px; border: 1px solid #ccd0d4; box-shadow: 0 1px 1px rgba(0,0,0,.04); margin-top: 20px; border-left: 4px solid #2271b1;">
            ...
            <form method="post" id="site-wide-discount">
                <?php wp_nonce_field('cta_site_wide_discount_action'); ?>
                ...
                <p class="submit">
                    <input type="submit" name="cta_save_site_wide_discount" class="button button-primary" value="Save Site-Wide Discount">
                </p>
            </form>
        ...
        <div style="background: #fff; padding: 20px; border: 1px solid #ccd0d4; box-shadow: 0 1px 1px rgba(0,0,0,.04); margin-top: 20px; border-left: 4px solid #00a32a;">
            ...
            <form method="post" id="course-discount">
                <?php wp_nonce_field('cta_course_discount_action'); ?>
                <table class="form-table">
                    <tr>
                ...
                <p class="submit">
                    <input type="submit" name="cta_save_course_discount" class="button button-primary" value="Save Course Discount" id="save-course-discount-btn">
        ...
        <div style="background: #fff; padding: 20px; border: 1px solid #ccd0d4; box-shadow: 0 1px 1px rgba(0,0,0,.04); margin-top: 20px;">
            ...
            <form method="post" id="add-code">
                <?php wp_nonce_field('cta_discount_codes_action'); ?>
                <table class="form-table">
                    <tr>
                ...
                <p class="submit">
                    <input type="submit" name="cta_add_discount_code" class="button button-primary" value="Add Discount Code">
                </p>
            </form>
        </div>
                ...
                <table class="wp-list-table widefat fixed striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="<?php echo esc_url(wp_nonce_url(add_query_arg(['toggle' => $index], admin_url('edit.php?post_type=course&page=cta-discount-codes')), 'cta_toggle_discount_code_' . $index)); ?>" class="button button-small">
                                ...
                                if (!empty($oauth_token) && !empty($org_id)) :
                                ?>
                                    <a href="<?php echo esc_url(wp_nonce_url(add_query_arg(['toggle_eventbrite' => $index], admin_url('edit.php?post_type=course&page=cta-discount-codes')), 'cta_toggle_eventbrite_sync_' . $index)); ?>" class="button button-small" style="margin-left: 5px;">
                                        <?php echo $sync_enabled ? 'Unsync' : 'Sync to EB'; ?>
                                    </a>
                                <?php endif; ?>
                                <a href="<?php echo esc_url(wp_nonce_url(add_query_arg(['delete' => $index], admin_url('edit.php?post_type=course&page=cta-discount-codes')), 'cta_delete_discount_code_' . $index)); ?>" class="button button-small" onclick="return confirm('Are you sure you want to delete this discount code?');" style="color: #b32d2e; margin-left: 5px;">
                                    Delete
                                </a>
                            </td>
                        </tr>
