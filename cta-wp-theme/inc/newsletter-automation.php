 ...
 */
function cta_create_automation_tables() {
    global $wpdb;
    ...
    cta_create_default_email_templates();
    
    // Create default automations if they don't exist
    cta_create_default_automations();
}
add_action('after_switch_theme', 'cta_create_automation_tables');
add_action('after_switch_theme', 'cta_create_default_email_templates', 20);
add_action('after_switch_theme', 'cta_create_default_automations', 25);
...
add_action('admin_init', function() {
    ...
    if ($templates_exist && $flows_exist) {
        ...
        $last_check = get_transient('cta_defaults_check');
        if (!$last_check) {
            cta_create_default_email_templates();
            cta_create_default_automations();
            set_transient('cta_defaults_check', true, DAY_IN_SECONDS);
        }
    } else if (!$templates_exist || !$flows_exist) {
        // Tables don't exist, create them
        cta_create_automation_tables();
    }
});
 ...
 */
function cta_create_default_email_templates() {
    global $wpdb;
    $templates_table = $wpdb->prefix . 'cta_email_templates';
 ...
 */
function cta_create_default_automations() {
    global $wpdb;
    $flows_table = $wpdb->prefix . 'cta_automation_flows';
    $steps_table = $wpdb->prefix . 'cta_automation_steps';
    $templates_table = $wpdb->prefix . 'cta_email_templates';
...
function cta_automation_admin_menu() {
    ...
    add_submenu_page(
        ...
        $cap,
        'cta-email-templates',
        'cta_email_templates_admin_page'
    );
}
 ...
 */
function cta_automation_admin_page() {
    global $wpdb;
    $flows_table = $wpdb->prefix . 'cta_automation_flows';
 ...
 */
function cta_email_templates_admin_page() {
    global $wpdb;
    $templates_table = $wpdb->prefix . 'cta_email_templates';
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'edit' && isset($_GET['template_id'])) {
        ...
        if ($template) {
            cta_render_template_builder($template);
            return;
        }
    }
