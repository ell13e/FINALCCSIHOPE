 ...
 */
function cta_linker_add_metabox() {
    add_meta_box(
        'cta_internal_linker',
        'Smart Internal Links',
        'cta_linker_metabox_callback',
        'post',
        'normal',
        'high'
    );
}
add_action('add_meta_boxes', 'cta_linker_add_metabox');

/**
 * Internal Linker meta box content
 */
function cta_linker_metabox_callback($post) {
    wp_nonce_field('cta_linker_nonce', 'cta_linker_nonce_field');
    ?>
    <style>
    ...
    jQuery(document).ready(function($) {
        ...
        $('#cta-linker-scan').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_linker_scan',
                    nonce: $('#cta_linker_nonce_field').val(),
                    content: content,
 ...
 */
function cta_linker_scan_ajax() {
    check_ajax_referer('cta_linker_nonce', 'nonce');
    ...
    foreach ($courses as $course) {
        $keywords = cta_extract_keywords($course->post_title);
    ...
    foreach ($posts as $p) {
        $keywords = cta_extract_keywords($p->post_title);
        foreach ($keywords as $keyword) {
            $link_targets[] = [
                'keyword' => $keyword,
                'title' => $p->post_title,
    ...
    foreach ($pages as $page) {
        $keywords = cta_extract_keywords($page->post_title);
        foreach ($keywords as $keyword) {
            $link_targets[] = [
                'keyword' => $keyword,
...
add_action('wp_ajax_cta_linker_scan', 'cta_linker_scan_ajax');

/**
 * Extract keywords from a title
 */
function cta_extract_keywords($title) {
    $keywords = [];
 ...
 */
function cta_auto_link_on_save($post_id, $post) {
    // Only for posts
    if ($post->post_type !== 'post') return;
    
    // Check if auto-linking is enabled
    if (!get_option('cta_auto_link_enabled', false)) return;
...
// add_action('save_post', 'cta_auto_link_on_save', 10, 2);
