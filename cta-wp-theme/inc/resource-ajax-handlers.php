...
function cta_ajax_request_resource_download() {
    ...
    if (!$resource_id || get_post_type($resource_id) !== 'cta_resource') {
    ...
    if ($phone !== '' && function_exists('cta_validate_uk_phone')) {
        $pv = cta_validate_uk_phone($phone);
    ...
    $expiry_days = (int) get_post_meta($resource_id, '_cta_resource_expiry_days', true);
    if ($expiry_days <= 0) $expiry_days = 7;
    if ($expiry_days > 30) $expiry_days = 30;
    $expires_at_ts = time() + ($expiry_days * DAY_IN_SECONDS);

    $token = function_exists('cta_resource_generate_download_token')
        ? cta_resource_generate_download_token($resource_id, $email, $expires_at_ts)
        : '';
    ...
    $subject = (string) get_post_meta($resource_id, '_cta_resource_email_subject', true);
    $body = (string) get_post_meta($resource_id, '_cta_resource_email_body', true);
    ...
    if ($consent && function_exists('cta_newsletter_add_subscriber')) {
        cta_newsletter_add_subscriber([
            'email' => $email,
            'first_name' => $first_name,
            'last_name' => $last_name,
            'phone' => $phone,
...
add_action('wp_ajax_cta_request_resource_download', 'cta_ajax_request_resource_download');
add_action('wp_ajax_nopriv_cta_request_resource_download', 'cta_ajax_request_resource_download');
