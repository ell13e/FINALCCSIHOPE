 ...
 */
function cta_register_resource_post_type() {
    $labels = [
        'name' => 'Resources',
    ...
    register_post_type('cta_resource', [
        'labels' => $labels,
        'public' => false,
        'show_ui' => true,
        'show_in_menu' => true,
    ...
    register_taxonomy('cta_resource_category', ['cta_resource'], [
...
add_action('init', 'cta_register_resource_post_type');

/**
 * Create downloads tracking table.
 */
function cta_create_resource_downloads_table() {
    global $wpdb;
    $table = $wpdb->prefix . 'cta_resource_downloads';
...
add_action('after_switch_theme', 'cta_create_resource_downloads_table', 40);

/**
 * Customize editor title label.
 */
function cta_resource_editor_title($title) {
    $screen = get_current_screen();
    if ($screen && $screen->post_type === 'cta_resource') {
        $title = 'Resource Description (displays on your website)';
    }
    return $title;
}
add_filter('enter_title_here', 'cta_resource_editor_title');

/**
 * Add custom admin CSS for resource pages
 */
function cta_resource_admin_styles() {
    $screen = get_current_screen();
    if (!$screen || $screen->post_type !== 'cta_resource' || $screen->base !== 'post') {
        return;
    }
    ?>
    <style>
...
add_action('admin_head', 'cta_resource_admin_styles');

/**
 * Add metaboxes.
 */
function cta_resource_add_meta_boxes() {
    add_meta_box(
        'cta_resource_file',
        'Resource File',
        'cta_resource_file_metabox',
        'cta_resource',
        'normal',
        'high'
    );
    ...
    add_meta_box(
        'cta_resource_email_template',
        'Email Template',
        'cta_resource_email_template_metabox',
        'cta_resource',
        'normal',
        'default'
    );
    ...
    add_meta_box(
        'cta_resource_settings',
        'Resource Settings',
        'cta_resource_settings_metabox',
        'cta_resource',
        'side',
        'default'
    );
}
add_action('add_meta_boxes', 'cta_resource_add_meta_boxes');

function cta_resource_file_metabox($post) {
    wp_nonce_field('cta_resource_meta', 'cta_resource_meta_nonce');
    $file_id = (int) get_post_meta($post->ID, '_cta_resource_file_id', true);
    $file_url = $file_id ? wp_get_attachment_url($file_id) : '';
    $file_name = $file_id ? basename(get_attached_file($file_id)) : '';
    ?>
    <div style="margin-bottom: 20px;">
        <p>
            <label for="cta_resource_file_id"><strong>Attachment ID</strong></label>
        </p>
        <p>
            <input type="number" class="regular-text" id="cta_resource_file_id" name="cta_resource_file_id" value="<?php echo esc_attr($file_id); ?>" placeholder="0" required>
        </p>
        <?php if ($file_url) : ?>
...
function cta_resource_email_template_metabox($post) {
    $subject = (string) get_post_meta($post->ID, '_cta_resource_email_subject', true);
    $body = (string) get_post_meta($post->ID, '_cta_resource_email_body', true);

    if ($subject === '') {
        $subject = 'Your {{resource_name}} from Continuity Training Academy';
    }
    if ($body === '') {
    ...
    <div style="margin-bottom: 20px;">
        <p>
            <label for="cta_resource_email_subject"><strong>Subject</strong></label>
        </p>
        <p>
            <input type="text" class="widefat" id="cta_resource_email_subject" name="cta_resource_email_subject" value="<?php echo esc_attr($subject); ?>" required placeholder="Your {{resource_name}} from Continuity Training Academy">
        </p>
    </div>
    <div style="margin-bottom: 20px;">
        <p>
            <label for="cta_resource_email_body"><strong>Body</strong></label>
        </p>
        <p>
            <textarea class="widefat" rows="12" id="cta_resource_email_body" name="cta_resource_email_body" required placeholder="Hi {{first_name}},&#10;&#10;Thank you for requesting {{resource_name}}..."><?php echo esc_textarea($body); ?></textarea>
        </p>
    </div>
...
function cta_resource_settings_metabox($post) {
    $icon = (string) get_post_meta($post->ID, '_cta_resource_icon', true);
    $expiry_days = (int) get_post_meta($post->ID, '_cta_resource_expiry_days', true);
    if ($expiry_days <= 0) $expiry_days = 7;
    if ($icon === '') $icon = 'fas fa-file';
    ?>
    <div style="margin-bottom: 16px;">
        <label for="cta_resource_icon"><strong>Icon Class</strong></label>
        <input type="text" class="widefat" id="cta_resource_icon" name="cta_resource_icon" value="<?php echo esc_attr($icon); ?>" placeholder="fas fa-file-pdf">
    ...
    <div style="margin-bottom: 16px;">
        <label for="cta_resource_expiry_days"><strong>Link Expiry (days)</strong></label>
        <input type="number" class="small-text" id="cta_resource_expiry_days" name="cta_resource_expiry_days" value="<?php echo esc_attr($expiry_days); ?>" min="1" max="30" required>
        <p class="description" style="margin-top: 6px;">
 ...
 */
function cta_resource_title_placeholder($title) {
    $screen = get_current_screen();
    if ($screen && $screen->post_type === 'cta_resource') {
...
add_filter('enter_title_here', 'cta_resource_title_placeholder');

/**
 * Customize the editor label
 */
function cta_resource_editor_settings($settings) {
    $screen = get_current_screen();
    if ($screen && $screen->post_type === 'cta_resource') {
        $settings['tinymce'] = [
...
add_filter('wp_editor_settings', 'cta_resource_editor_settings');

/**
 * Add helpful admin notices for resource creation
 */
function cta_resource_admin_notices() {
    $screen = get_current_screen();
    if (!$screen || $screen->post_type !== 'cta_resource') {
        return;
    }
    ...
    if ($is_new) {
        ...
        $file_id = get_post_meta($post->ID, '_cta_resource_file_id', true);
        $warnings = [];
        ...
        $categories = wp_get_post_terms($post->ID, 'cta_resource_category');
...
add_action('admin_notices', 'cta_resource_admin_notices');

/**
 * Save resource meta.
 */
function cta_resource_save_meta($post_id) {
    if (!isset($_POST['cta_resource_meta_nonce']) || !wp_verify_nonce($_POST['cta_resource_meta_nonce'], 'cta_resource_meta')) {
        return;
    }
    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) {
        return;
    }
    if (get_post_type($post_id) !== 'cta_resource') {
        return;
    }
    ...
    $file_id = absint($_POST['cta_resource_file_id'] ?? 0);
    update_post_meta($post_id, '_cta_resource_file_id', $file_id);

    $subject = sanitize_text_field($_POST['cta_resource_email_subject'] ?? '');
    $body = wp_kses_post($_POST['cta_resource_email_body'] ?? '');
    update_post_meta($post_id, '_cta_resource_email_subject', $subject);
    update_post_meta($post_id, '_cta_resource_email_body', $body);

    $icon = sanitize_text_field($_POST['cta_resource_icon'] ?? '');
    $expiry_days = absint($_POST['cta_resource_expiry_days'] ?? 7);
    if ($expiry_days < 1) $expiry_days = 1;
    if ($expiry_days > 30) $expiry_days = 30;

    update_post_meta($post_id, '_cta_resource_icon', $icon);
    update_post_meta($post_id, '_cta_resource_expiry_days', $expiry_days);
}
add_action('save_post', 'cta_resource_save_meta');

/**
 * Enqueue AI assistant script for resource edit screen
 */
function cta_resource_ai_assistant_scripts($hook) {
    global $post;
    ...
    if (!$post || get_post_type($post) !== 'cta_resource') {
        return;
    }
    ...
    jQuery(document).ready(function($) {
        $('#cta-resource-ai-assistant').on('click', function() {
            ...
            const fileId = $('#cta_resource_file_id').val();
            let fileName = '';
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_generate_resource_content',
                    nonce: '<?php echo wp_create_nonce('cta_resource_ai_nonce'); ?>',
                    file_name: fileName
                },
                success: function(response) {
                    if (response.success && response.data) {
                        ...
                        if (response.data.email_subject) {
                            $('#cta_resource_email_subject').val(response.data.email_subject);
                        }
                        
                        // Fill in email body
                        if (response.data.email_body) {
                            $('#cta_resource_email_body').val(response.data.email_body);
                        }
                        
                        // Fill in icon suggestion
                        if (response.data.icon) {
                            $('#cta_resource_icon').val(response.data.icon);
                        }
...
add_action('admin_footer', 'cta_resource_ai_assistant_scripts');

/**
 * AJAX handler to generate resource content with AI
 */
function cta_ajax_generate_resource_content() {
    check_ajax_referer('cta_resource_ai_nonce', 'nonce');
    ...
    if (!$content) {
        ...
        $content = [
            ...
            'email_subject' => 'Your {{resource_name}} from Continuity Training Academy',
...
add_action('wp_ajax_cta_generate_resource_content', 'cta_ajax_generate_resource_content');
