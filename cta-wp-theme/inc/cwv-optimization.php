 ...
 */
function cta_get_responsive_image_html($attachment_id, $size = 'medium', $args = []) {
    $defaults = [
        'alt' => '',
        'class' => '',
        'lazy' => true,
    ...
    if (is_string($attachment_id)) {
        ...
        if (!$attachment_id) {
            ...
            return cta_get_simple_image_html($image_url, $size, $args);
        }
    } else {
 ...
 */
function cta_get_simple_image_html($url, $size = 'medium', $args = []) {
    $defaults = [
        'alt' => '',
        'class' => '',
        'lazy' => true,
 ...
 * @return void
 */
function cta_output_resource_hints($resources = []) {
    $defaults = [
        'preload' => [],
        'prefetch' => [],
        'dns-prefetch' => [],
 ...
 * @return array Preload resource hints for cta_output_resource_hints()
 */
function cta_get_hero_image_preload_hints($attachment_id, $include_webp = true) {
    if (!$attachment_id) {
        return [];
    }
 ...
 */
function cta_ensure_image_dimensions($html, $images_with_dims = []) {
 ...
 */
function cta_get_lazy_image_placeholder_css() {
    return '
    <style>
    /* Lazy-loaded images */
    img[loading="lazy"] {
 ...
 * @return void
 */
function cta_output_performance_monitoring() {
    ?>
    <script>
    (function() {
    ...
    function cta_sendMetric(metric, value) {
        if (navigator.sendBeacon) {
            navigator.sendBeacon('<?php echo admin_url('admin-ajax.php'); ?>', {
                action: 'cta_log_performance_metric',
                metric: metric,
                value: value,
                url: window.location.href,
                nonce: '<?php echo wp_create_nonce('cta_performance_nonce'); ?>'
            });
        }
    }
    </script>
 ...
 */
function cta_handle_log_performance_metric() {
    // Verify nonce
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'cta_performance_nonce')) {
...
add_action('wp_ajax_nopriv_cta_log_performance_metric', 'cta_handle_log_performance_metric');
add_action('wp_ajax_cta_log_performance_metric', 'cta_handle_log_performance_metric');
 ...
 * @return void
 */
function cta_output_critical_css($critical_css = '') {
 ...
 * @return void
 */
function cta_defer_non_critical_css($handle) {
