 ...
 */
function cta_get_event_schema($post_id = null) {
    if (!$post_id) {
        $post_id = get_the_ID();
    }
    ...
    $linked_course = cta_safe_get_field('linked_course', $post_id, null);
    
    // Date/time fields
    $event_date = cta_safe_get_field('event_date', $post_id, '');
    $start_time = cta_safe_get_field('start_time', $post_id, '09:00');
    $end_time = cta_safe_get_field('end_time', $post_id, '17:00');
    $event_status = cta_safe_get_field('event_status', $post_id, 'Scheduled');
    $event_attendance_mode = cta_safe_get_field('event_attendance_mode', $post_id, 'OfflineEventAttendanceMode');
    
    // Location
    $location_name = cta_safe_get_field('event_location', $post_id, 'Maidstone');
    $location_address = cta_safe_get_field('event_location_address', $post_id, '');
    
    // Pricing & Availability
    $event_price = cta_safe_get_field('event_price', $post_id, '');
    $spaces_available = cta_safe_get_field('spaces_available', $post_id, '');
    $currency = 'GBP';
    ...
    if (!empty($event_date)) {
        $start_datetime = cta_format_event_datetime($event_date, $start_time);
        if ($start_datetime) {
            $schema['startDate'] = $start_datetime;
        }
        
        if (!empty($end_time)) {
            $end_datetime = cta_format_event_datetime($event_date, $end_time);
            if ($end_datetime) {
    ...
    $latitude = get_theme_mod('cta_seo_geo_lat', '51.264494');
    $longitude = get_theme_mod('cta_seo_geo_lng', '0.545844');
    ...
    $performer_name = cta_safe_get_field('event_instructor', $post_id, '');
 ...
 */
function cta_format_event_datetime($date, $time = '09:00') {
 ...
 */
function cta_get_event_collection_schema($event_ids = []) {
    $site_url = home_url();
    ...
    foreach ($event_ids as $event_id) {
        ...
        $event_schema = [
            ...
            'startDate' => cta_get_event_start_date_iso($event_id),
        ];
 ...
 */
function cta_get_event_start_date_iso($post_id) {
    $event_date = cta_safe_get_field('event_date', $post_id, '');
    $start_time = cta_safe_get_field('start_time', $post_id, '09:00');
    
    return cta_format_event_datetime($event_date, $start_time);
}
 ...
 * @return void
 */
function cta_output_event_schema($post_id = null, $include_organization = true) {
    if (!$post_id) {
        $post_id = get_the_ID();
    }
    ...
    $schema_graph[] = cta_get_breadcrumb_schema($breadcrumb_items);

    // Add event schema
    $schema_graph[] = cta_get_event_schema($post_id);
 ...
 * @return void
 */
function cta_output_event_collection_schema($event_ids = []) {
    $schema_graph = [];
    ...
    // Add collection
    $schema_graph[] = cta_get_event_collection_schema($event_ids);
 ...
 */
function cta_hook_output_event_schema() {
    if (is_singular('course_event')) {
        cta_output_event_schema(null, true);
    }
}
add_action('wp_head', 'cta_hook_output_event_schema', 15); // After open tag, before other meta

/**
 * Hook: Output event collection schema on event archive
 */
function cta_hook_output_event_archive_schema() {
    if (is_post_type_archive('course_event')) {
        cta_output_event_collection_schema();
    }
}
add_action('wp_head', 'cta_hook_output_event_archive_schema', 15);
