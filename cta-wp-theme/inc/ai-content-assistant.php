 ...
 */
function cta_ai_register_settings() {
    register_setting('cta_ai_settings', 'cta_ai_api_key', [
        'sanitize_callback' => 'sanitize_text_field',
        'type' => 'string',
    ]);
    // Provider-specific keys (optional, used for fallback order)
    register_setting('cta_ai_settings', 'cta_ai_groq_api_key', [
        'sanitize_callback' => 'sanitize_text_field',
        'type' => 'string',
    ]);
    register_setting('cta_ai_settings', 'cta_ai_anthropic_api_key', [
        'sanitize_callback' => 'sanitize_text_field',
        'type' => 'string',
    ]);
    register_setting('cta_ai_settings', 'cta_ai_openai_api_key', [
        'sanitize_callback' => 'sanitize_text_field',
        'type' => 'string',
    ]);
    register_setting('cta_ai_settings', 'cta_ai_provider', [
        'sanitize_callback' => 'sanitize_text_field',
        'default' => 'groq',
        'type' => 'string',
    ]);
}
add_action('admin_init', 'cta_ai_register_settings');

/**
 * Add AI Settings page under Settings menu
 */
function cta_ai_settings_menu() {
    add_options_page(
        'AI Content Assistant',
        'AI Assistant',
        'manage_options',
        'cta-ai-settings',
        'cta_ai_settings_page'
    );
}
add_action('admin_menu', 'cta_ai_settings_menu');

/**
 * AI Settings page content
 */
function cta_ai_settings_page() {
    ...
    <div class="wrap">
        ...
        <form method="post" action="options.php">
            <?php settings_fields('cta_ai_settings'); ?>
            ...
            <div class="cta-ai-settings-section">
                ...
                <div class="cta-ai-settings-field">
                    <label for="cta_ai_provider">AI Provider</label>
                    <select name="cta_ai_provider" id="cta_ai_provider">
                            <option value="groq" <?php selected(get_option('cta_ai_provider', 'groq'), 'groq'); ?>>Groq (Fast)</option>
                            <option value="anthropic" <?php selected(get_option('cta_ai_provider', 'groq'), 'anthropic'); ?>>Claude (Anthropic) - Best for writing</option>
                            <option value="openai" <?php selected(get_option('cta_ai_provider'), 'openai'); ?>>GPT-4 (OpenAI)</option>
                        </select>
                ...
                <div class="cta-ai-settings-field">
                    <label for="cta_ai_api_key">API Key</label>
                    <input 
                        type="password" 
                        name="cta_ai_api_key" 
                        id="cta_ai_api_key" 
                        value="<?php echo esc_attr(get_option('cta_ai_api_key')); ?>" 
                        autocomplete="off"
                        spellcheck="false"
                        class="regular-text"
                    />
                ...
                <div class="cta-ai-settings-field">
                    <label for="cta_ai_groq_api_key">Groq API Key</label>
                    <input
                        type="password"
                        name="cta_ai_groq_api_key"
                        id="cta_ai_groq_api_key"
                        value="<?php echo esc_attr(get_option('cta_ai_groq_api_key')); ?>"
                        autocomplete="off"
                        spellcheck="false"
                        class="regular-text"
                    />
                ...
                <div class="cta-ai-settings-field">
                    <label for="cta_ai_anthropic_api_key">Claude (Anthropic) API Key</label>
                    <input
                        type="password"
                        name="cta_ai_anthropic_api_key"
                        id="cta_ai_anthropic_api_key"
                        value="<?php echo esc_attr(get_option('cta_ai_anthropic_api_key')); ?>"
                        autocomplete="off"
                        spellcheck="false"
                        class="regular-text"
                    />
                ...
                <div class="cta-ai-settings-field">
                    <label for="cta_ai_openai_api_key">OpenAI API Key</label>
                    <input
                        type="password"
                        name="cta_ai_openai_api_key"
                        id="cta_ai_openai_api_key"
                        value="<?php echo esc_attr(get_option('cta_ai_openai_api_key')); ?>"
                        autocomplete="off"
                        spellcheck="false"
                        class="regular-text"
                    />
 ...
 */
function cta_ai_add_metabox() {
    add_meta_box(
        'cta_ai_assistant',
        '🤖 AI Content Assistant',
        'cta_ai_metabox_callback',
        'post',
        'side',
        'high'
    );
    ...
    add_meta_box(
        'cta_ai_course_assistant',
        '🤖 AI Course Assistant',
        'cta_ai_course_metabox_callback',
        'course',
        'side',
        'high'
    );
    ...
    add_meta_box(
        'cta_ai_event_assistant',
        '🤖 AI Event Assistant',
        'cta_ai_event_metabox_callback',
        'course_event',
        'side',
        'high'
    );
}
add_action('add_meta_boxes', 'cta_ai_add_metabox');

/**
 * AI Assistant meta box content
 */
function cta_ai_metabox_callback($post) {
    ...
    wp_nonce_field('cta_ai_nonce', 'cta_ai_nonce_field');
    ?>
    <style>
        .cta-ai-section {
            margin-bottom: 16px;
    ...
    jQuery(document).ready(function($) {
        ...
        $('#cta-ai-generate').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_ai_generate',
                    nonce: $('#cta_ai_nonce_field').val(),
                    topic: topic,
                    tone: tone,
                    type: 'article'
                },
 ...
 */
function cta_ai_course_metabox_callback($post) {
    $api_key = get_option('cta_ai_api_key');
 ...
 */
function cta_ai_event_metabox_callback($post) {
    $api_key = get_option('cta_ai_api_key');
 ...
 */
function cta_ai_generate_ajax() {
    check_ajax_referer('cta_ai_nonce', 'nonce');
    ...
    $company_knowledge = function_exists('cta_get_ai_company_context') ? cta_get_ai_company_context() : '';
    
    // Build prompt based on type
    $system_prompt = "ROLE: Expert content writer for Continuity Training Academy (care sector training provider, Kent, UK)
...
4. Write conclusion (100-150 words) with:
   ...
   - Call-to-action mentioning Continuity Training Academy courses
...
{$content}";
    ...
    $result = cta_ai_try_providers($preferred_provider, function($provider, $api_key) use ($system_prompt, $prompt) {
        if ($provider === 'groq') {
            return function_exists('cta_call_groq_api') ? cta_call_groq_api($api_key, $system_prompt, $prompt) : new WP_Error('missing_fn', 'Groq function missing');
        }
        if ($provider === 'anthropic') {
            return function_exists('cta_call_anthropic_api') ? cta_call_anthropic_api($api_key, $system_prompt, $prompt) : new WP_Error('missing_fn', 'Anthropic function missing');
        }
        return function_exists('cta_call_openai_api') ? cta_call_openai_api($api_key, $system_prompt, $prompt) : new WP_Error('missing_fn', 'OpenAI function missing');
    });
...
add_action('wp_ajax_cta_ai_generate', 'cta_ai_generate_ajax');

/**
 * AJAX handler for course content generation
 */
function cta_ai_generate_course_ajax() {
    check_ajax_referer('cta_ai_nonce', 'nonce');
    ...
    $system_prompt = "ROLE: Expert course content writer for Continuity Training Academy (care sector training provider, Kent, UK)
...
- Do NOT reference sources outside this approved list unless explicitly asked";
    ...
    $result = cta_ai_try_providers($preferred_provider, function($provider, $api_key) use ($system_prompt, $prompt) {
        if ($provider === 'groq') return cta_call_groq_api($api_key, $system_prompt, $prompt);
        if ($provider === 'anthropic') return cta_call_anthropic_api($api_key, $system_prompt, $prompt);
    ...
    $parsed = cta_parse_ai_response_with_suggestions($result);
...
add_action('wp_ajax_cta_ai_generate_course', 'cta_ai_generate_course_ajax');

/**
 * AJAX handler for event content generation
 */
function cta_ai_generate_event_ajax() {
    check_ajax_referer('cta_ai_nonce', 'nonce');
    ...
    $system_prompt = "You are an expert event content writer for Continuity Training Academy, a care sector training provider in Kent, UK.
...
add_action('wp_ajax_cta_ai_generate_event', 'cta_ai_generate_event_ajax');

/**
 * Parse AI response to extract content, suggestions, and follow-ups
 */
function cta_parse_ai_response_with_suggestions($response) {
    $content = $response;
    $suggestions = [];
    $followups = [];
 ...
 */
function cta_call_anthropic_api($api_key, $system, $prompt) {
 ...
 */
function cta_call_openai_api($api_key, $system, $prompt) {
 ...
 */
function cta_call_groq_api($api_key, $system, $prompt) {
