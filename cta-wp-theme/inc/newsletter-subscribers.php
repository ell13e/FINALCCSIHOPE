 ...
 */
function cta_newsletter_required_capability() {
 ...
 */
function cta_newsletter_user_can_manage() {
    return current_user_can(cta_newsletter_required_capability());
}

/**
 * Create newsletter tag tables (tags + subscriber ↔ tag relationships).
 */
function cta_create_newsletter_tag_tables() {
    global $wpdb;
    $charset_collate = $wpdb->get_charset_collate();

    $tags_table = $wpdb->prefix . 'cta_newsletter_tags';
    $rel_table = $wpdb->prefix . 'cta_newsletter_subscriber_tags';
 ...
 */
function cta_newsletter_get_tags() {
    global $wpdb;
    $tags_table = $wpdb->prefix . 'cta_newsletter_tags';
 ...
 */
function cta_newsletter_get_tag_ids_for_subscribers($subscriber_ids) {
 ...
 */
function cta_newsletter_assign_tags($subscriber_ids, $tag_ids) {
    $subscriber_ids = array_values(array_filter(array_map('absint', (array) $subscriber_ids)));
 ...
 */
function cta_newsletter_remove_tags($subscriber_ids, $tag_ids) {
    $subscriber_ids = array_values(array_filter(array_map('absint', (array) $subscriber_ids)));
    $tag_ids = array_values(array_filter(array_map('absint', (array) $tag_ids)));
    if (empty($subscriber_ids) || empty($tag_ids)) return 0;
 ...
 */
function cta_newsletter_create_tag($name, $color = '') {
    global $wpdb;
    $tags_table = $wpdb->prefix . 'cta_newsletter_tags';
 ...
 */
function cta_create_subscribers_table() {
    global $wpdb;
    
    $table_name = $wpdb->prefix . 'cta_newsletter_subscribers';
    $charset_collate = $wpdb->get_charset_collate();
    ...
    cta_verify_subscriber_ids();
    
    // Create tracking tables
    cta_create_email_tracking_tables();

    // Create tags tables
    cta_create_newsletter_tag_tables();
    
    // Create unsubscribe page if it doesn't exist
    cta_create_unsubscribe_page();
}
add_action('after_switch_theme', 'cta_create_subscribers_table');
add_action('after_switch_theme', 'cta_migrate_existing_data_to_newsletter', 20);

/**
 * Create unsubscribe page if it doesn't exist
 */
function cta_create_unsubscribe_page() {
 ...
 */
function cta_create_email_tracking_tables() {
    global $wpdb;
    $charset_collate = $wpdb->get_charset_collate();
    
    // Campaigns table
    $campaigns_table = $wpdb->prefix . 'cta_email_campaigns';
    ...
    $opens_table = $wpdb->prefix . 'cta_email_opens';
    ...
    $clicks_table = $wpdb->prefix . 'cta_email_clicks';
    ...
    $queue_table = $wpdb->prefix . 'cta_email_queue';
...
function cta_maybe_create_subscribers_table() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'cta_newsletter_subscribers';
    
    if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") !== $table_name) {
        cta_create_subscribers_table();
    }
}
add_action('admin_init', 'cta_maybe_create_subscribers_table');
 ...
 */
function cta_add_newsletter_subscriber($email, $ip = '', $first_name = '', $last_name = '', $date_of_birth = '') {
    global $wpdb;
    $table_name = $wpdb->prefix . 'cta_newsletter_subscribers';
    ...
    if ($insert_result === false) {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('CTA Newsletter: Failed to insert subscriber - ' . $wpdb->last_error);
        }
    ...
    if (empty($subscriber_id)) {
        ...
        if (empty($subscriber_id)) {
            if (defined('WP_DEBUG') && WP_DEBUG) {
                error_log('CTA Newsletter: Subscriber inserted but ID not found');
            }
            return false;
        }
    }
 ...
 */
function cta_get_newsletter_subscribers($status = 'active') {
    global $wpdb;
    $table_name = $wpdb->prefix . 'cta_newsletter_subscribers';
 ...
 */
function cta_verify_subscriber_ids() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'cta_newsletter_subscribers';
    ...
    if (!empty($subscribers_without_id)) {
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('CTA Newsletter: Found ' . count($subscribers_without_id) . ' subscribers without IDs. This should not happen with AUTO_INCREMENT.');
        }
    ...
    if ($column_info && strpos($column_info->Extra, 'auto_increment') === false) {
        ...
        if (defined('WP_DEBUG') && WP_DEBUG) {
            error_log('CTA Newsletter: id column missing AUTO_INCREMENT. Attempting to fix...');
        }
 ...
 */
function cta_get_subscriber_count($status = 'active') {
    global $wpdb;
    $table_name = $wpdb->prefix . 'cta_newsletter_subscribers';
 ...
 */
function cta_import_newsletter_csv($file) {
    global $wpdb;
    $table_name = $wpdb->prefix . 'cta_newsletter_subscribers';
    
    // Security check
    if (!cta_newsletter_user_can_manage()) {
 ...
 */
function cta_newsletter_admin_enqueue_styles($hook) {
...
add_action('admin_enqueue_scripts', 'cta_newsletter_admin_enqueue_styles');

/**
 * Add admin menu page
 */
function cta_newsletter_admin_menu() {
    $cap = cta_newsletter_required_capability();
    add_menu_page(
        'Newsletter Subscribers',
        'Newsletter',
        $cap,
        'cta-newsletter',
        'cta_newsletter_admin_page',
        'dashicons-email-alt',
        30
    );
    ...
    add_submenu_page(
        ...
        'cta_newsletter_subscribers_page'
    );
    add_submenu_page(
        'cta-newsletter',
        'Newsletter Calendar',
    ...
    add_submenu_page(
        ...
        $cap,
        'cta-newsletter-tags',
        'cta_newsletter_tags_page'
    );
}
add_action('admin_menu', 'cta_newsletter_admin_menu');
 ...
 */
function cta_newsletter_compose_page() {
    if (!cta_newsletter_user_can_manage()) {
        wp_die('You do not have permission to access this page.');
    }

    // Handle send
    if (isset($_POST['cta_send_newsletter']) && check_admin_referer('cta_send_newsletter')) {
        cta_send_newsletter_email();
    }
    ...
    $active_count = cta_get_subscriber_count('active');
    $all_tags = cta_newsletter_get_tags();
    ...
    <div class="wrap">
        ...
        <?php cta_render_newsletter_navigation('cta-newsletter-compose'); ?>
        ...
        <form method="post" id="newsletter-form">
            <?php wp_nonce_field('cta_send_newsletter'); ?>
            ...
            <div class="postbox">
                ...
                <div class="inside">
                    <p>
                        <button type="submit" class="button button-primary" name="cta_send_newsletter" value="1">Send / Schedule</button>
...
function cta_newsletter_campaigns_page() {
    if (!cta_newsletter_user_can_manage()) {
        wp_die('You do not have permission to access this page.');
    }
    ...
    if (isset($_GET['action'], $_GET['campaign'])) {
        ...
        if ($campaign_id > 0 && $action === 'cancel_scheduled' && check_admin_referer('cancel_scheduled_' . $campaign_id)) {
            $cancelled = cta_cancel_scheduled_campaign($campaign_id);
            if ($cancelled) {
        ...
        if ($campaign_id > 0 && $action === 'retry' && check_admin_referer('retry_failed_' . $campaign_id)) {
            $retried = cta_retry_failed_emails($campaign_id);
            if ($retried > 0) {
    ...
    <div class="wrap">
        ...
        <?php cta_render_newsletter_navigation('cta-newsletter-campaigns'); ?>
...
function cta_newsletter_subscribers_page() {
    if (!cta_newsletter_user_can_manage()) {
        wp_die('You do not have permission to access this page.');
    }
    ...
    if (isset($_POST['remove_all_unsubscribed']) && check_admin_referer('cta_remove_all_unsubscribed')) {
    ...
    // Handle bulk actions
    if (isset($_POST['bulk_action']) && isset($_POST['subscriber_ids']) && is_array($_POST['subscriber_ids']) && check_admin_referer('cta_newsletter_bulk_action')) {
    ...
    // Handle CSV import
    if (isset($_POST['cta_import_newsletter_csv']) && check_admin_referer('cta_import_newsletter_csv')) {
        if (isset($_FILES['csv_file']) && $_FILES['csv_file']['error'] === UPLOAD_ERR_OK) {
            $result = cta_import_newsletter_csv($_FILES['csv_file']);
    ...
    $subscriber_tag_ids_map = cta_newsletter_get_tag_ids_for_subscribers($subscribers_on_page_ids);
...
function cta_newsletter_calendar_page() {
    if (!cta_newsletter_user_can_manage()) {
        wp_die('You do not have permission to access this page.');
    }
    ...
    <div class="wrap">
        ...
        <?php cta_render_newsletter_navigation('cta-newsletter-calendar'); ?>
...
function cta_newsletter_tags_page() {
    if (!cta_newsletter_user_can_manage()) {
        wp_die('You do not have permission to access this page.');
    }
    ...
    // Create tag
    if (isset($_POST['cta_create_tag']) && check_admin_referer('cta_create_tag')) {
        $name = sanitize_text_field($_POST['tag_name'] ?? '');
        $color = sanitize_text_field($_POST['tag_color'] ?? '');
        $res = cta_newsletter_create_tag($name, $color);
        if (is_wp_error($res)) {
    ...
    if (isset($_GET['action'], $_GET['tag_id']) && $_GET['action'] === 'delete') {
        ...
        if ($tag_id > 0 && check_admin_referer('cta_delete_tag_' . $tag_id)) {
    ...
    <div class="wrap">
        ...
        <?php cta_render_newsletter_navigation('cta-newsletter-tags'); ?>
        ...
        <div class="postbox" style="margin-top: 16px;">
            ...
            <div class="inside">
                <form method="post" style="display:flex; gap: 12px; align-items: end; flex-wrap: wrap;">
                    <?php wp_nonce_field('cta_create_tag'); ?>
                    <div>
                    ...
                    <div>
                        <button type="submit" class="button button-primary" name="cta_create_tag" value="1">Add tag</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <tbody>
                ...
                <?php else : ?>
                    <?php foreach ($tags as $t) : ?>
                        ...
                        $delete_url = wp_nonce_url(admin_url('admin.php?page=cta-newsletter-tags&action=delete&tag_id=' . $tag_id), 'cta_delete_tag_' . $tag_id);
                        ?>
                        <tr>
                            <td class="column-primary">
 ...
 */
function cta_render_newsletter_navigation($active_page = 'cta-newsletter') {
    $tabs = [
 ...
 * @deprecated Use cta_render_newsletter_navigation() instead
 */
function cta_render_newsletter_admin_primary_nav($active_page = 'cta-newsletter') {
    cta_render_newsletter_navigation($active_page);
}

/**
 * Newsletter Overview screen (clean WP admin UI).
 */
function cta_newsletter_render_overview_screen($active_count, $total_count, $unsubscribed_count, $total_sent, $recent_campaigns) {
    global $wpdb;
    $campaigns_table = $wpdb->prefix . 'cta_email_campaigns';
    ...
    $last_30_days_count = $wpdb->get_var("
        SELECT COUNT(*) 
        FROM {$wpdb->prefix}cta_newsletter_subscribers 
    ...
    <div class="wrap cta-newsletter-dashboard">
        ...
        <?php cta_render_newsletter_navigation('cta-newsletter'); ?>
 ...
 * Admin page content
 */
function cta_newsletter_admin_page() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'cta_newsletter_subscribers';
    $tags_table = $wpdb->prefix . 'cta_newsletter_tags';
    $rel_table = $wpdb->prefix . 'cta_newsletter_subscriber_tags';
        ...
        if ($action === 'unsubscribe' && !empty($ids)) {
            ...
            if ($tag_id <= 0) {
                ...
                if ($action === 'add_tag') {
                    ...
                    $deleted = cta_newsletter_remove_tags($ids, [$tag_id]);
    ...
    $active_count = cta_get_subscriber_count('active');
    $unsubscribed_count = cta_get_subscriber_count('unsubscribed');
    ...
    cta_newsletter_render_overview_screen($active_count, $total_count, $unsubscribed_count, $total_sent, $recent_campaigns);
    return;
    ...
    <div class="wrap cta-newsletter-wrap">
        ...
        <?php cta_render_newsletter_admin_primary_nav('cta-newsletter'); ?>
            ...
            <div id="tab-campaigns" class="cta-tab-content" style="display: none; padding: 24px;">
                ...
                if ($view_queue && $queue_campaign_id) {
                    ...
                    if (isset($_GET['action']) && $_GET['action'] === 'retry' && check_admin_referer('retry_failed_' . $queue_campaign_id)) {
                        $retried = cta_retry_failed_emails($queue_campaign_id);
                        if ($retried > 0) {
        ...
        function loadSavedTemplate(templateId) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_get_saved_template',
                    template_id: templateId,
                    nonce: '<?php echo wp_create_nonce('cta_newsletter_nonce'); ?>'
                },
        ...
        function fetchRecipientsForTags(tagIds, cb) {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_newsletter_get_recipients_for_tags',
                    nonce: '<?php echo wp_create_nonce('cta_newsletter_nonce'); ?>',
                    tag_ids: tagIds
                },
        ...
        <div class="postbox cta-subscribers-list">
            ...
            <div id="csv-import-form" style="display: none; background: #f9f9f9; padding: 20px; border-radius: 8px; margin-bottom: 20px; border: 1px solid #e0e0e0;">
                ...
                <form method="post" enctype="multipart/form-data">
                    ...
                    <div>
                        <button type="submit" name="cta_import_newsletter_csv" class="button button-primary">Import</button>
                ...
                <div class="notice notice-warning" style="margin: 16px 0;">
                    ...
                    <div style="margin-top: 10px;">
                        <form method="post" style="display: inline;" onsubmit="return confirm('Are you sure you want to permanently delete all unsubscribed subscribers? This cannot be undone.');">
                            <?php wp_nonce_field('cta_remove_all_unsubscribed'); ?>
    ...
    jQuery(document).ready(function($) {
        ...
        $(document).ajaxError(function(event, xhr, settings, thrownError) {
            if (settings.url && settings.url.includes('cta_')) {
        ...
        $('#cta-generate-email-subject').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_generate_email_subject',
                    content: emailContent || emailType,
                    nonce: '<?php echo wp_create_nonce('cta_ai_email'); ?>'
                },
                success: function(response) {
        ...
        $('#cta-generate-email-content').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_generate_newsletter_email',
        ...
        $('#email_type').on('change', function() {
            ...
            if (smartTemplates.indexOf(selected) !== -1) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        action: 'cta_get_populated_template',
        ...
        $('#cta-create-tag-btn').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_newsletter_create_tag',
                    nonce: '<?php echo wp_create_nonce('cta_newsletter_nonce'); ?>',
                    name: name,
                    color: color
                },
        ...
        $('#cta-tags-modal-save').on('click', function() {
            ...
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'cta_newsletter_update_subscriber_tags',
                    nonce: '<?php echo wp_create_nonce('cta_newsletter_nonce'); ?>',
        ...
        $confirmBtn.on('click', function() {
            if ($checkboxes.filter(':checked').length === $checkboxes.length) {
                ...
                $form.append('<input type="hidden" name="cta_send_newsletter" value="1">');
                $form.submit();
            }
        });
        ...
        $('#preview_email').on('click', function() {
            ...
            preview.document.write('<p><strong>Continuity Training Academy</strong><br>The Maidstone Studios, New Cut Road<br>Maidstone, Kent, ME14 5NZ</p>');
...
<?php
} // End cta_newsletter_admin_page()

/**
 * Send newsletter email to all active subscribers
 */
function cta_send_newsletter_email() {
    if (!cta_newsletter_user_can_manage()) {
        return;
    }
    ...
    if ($is_test_email) {
        ...
        cta_send_test_email($subject, $content, $link_type, $course_id, $article_id, $test_email_address);
        return;
    }
    ...
    if (!empty($recipient_ids)) {
        ...
        $subscribers = cta_get_newsletter_subscribers('active');
    }
    ...
    $from_email = defined('CTA_ENQUIRIES_EMAIL') && !empty(CTA_ENQUIRIES_EMAIL) 
        ? CTA_ENQUIRIES_EMAIL 
    ...
    if (empty($from_email)) {
        echo '<div class="notice notice-error"><p>❌ Email not configured. Please set CTA_ENQUIRIES_EMAIL in wp-config.php or configure WordPress admin email.</p></div>';
        return;
    }
    ...
    foreach ($subscribers as $subscriber) {
        ...
        $email_content = preg_replace_callback(
            ...
            function($matches) use ($campaign_id, $subscriber, $site_name) {
                ...
                if (strpos($url, 'cta_track') !== false || strpos($url, 'cta_unsubscribe') !== false) {
                    return $matches[0];
                }
        ...
        if ($use_queue) {
            ...
            if ($queued) {
                ...
                $failed_count++;
                if (defined('WP_DEBUG') && WP_DEBUG) {
                    error_log('CTA Newsletter: Failed to add email to queue for ' . $subscriber->email);
                }
            }
...
<body style="margin: 0; padding: 0; background-color: #f5f5f5; font-family: -apple-system, BlinkMacSystemFont, \'Segoe UI\', Roboto, \'Helvetica Neue\', Arial, sans-serif; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale;">
                    ...
                    <tr>
                        <td class="email-footer" style="padding: 24px 40px; background-color: #f9f9f9; border-top: 1px solid #e0e0e0; border-radius: 0 0 8px 8px;">
                            <p style="margin: 0 0 12px 0; font-size: 13px; color: #646970; text-align: center; line-height: 1.6;">
                                <strong>Continuity Training Academy</strong><br>
    ...
    <![endif]-->
        ...
        if ($sent) {
            ...
            $failed_count++;
            $error_message = 'CTA Newsletter: Failed to send to ' . $subscriber->email;
            if (defined('WP_DEBUG') && WP_DEBUG) {
    ...
    if ($use_queue && !$is_scheduled) {
        ...
        if (!wp_next_scheduled('cta_process_email_queue')) {
            wp_schedule_single_event(time() + 10, 'cta_process_email_queue');
        }
    } elseif ($is_scheduled) {
 ...
 */
function cta_handle_email_tracking() {
    if (!isset($_GET['cta_track']) || !isset($_GET['campaign']) || !isset($_GET['subscriber']) || !isset($_GET['token'])) {
        return;
    }
    
    global $wpdb;
    $track_type = sanitize_text_field($_GET['cta_track']);
    $campaign_id = absint($_GET['campaign']);
    $subscriber_id = absint($_GET['subscriber']);
    
    // Verify token
    $subscriber = $wpdb->get_row($wpdb->prepare(
        "SELECT email FROM {$wpdb->prefix}cta_newsletter_subscribers WHERE id = %d",
        $subscriber_id
    ));
...
add_action('template_redirect', 'cta_handle_email_tracking');

/**
 * Handle unsubscribe requests from email links
 */
function cta_handle_unsubscribe() {
    if (!isset($_GET['cta_unsubscribe']) || !isset($_GET['email']) || !isset($_GET['token'])) {
        return;
    }
    ...
    $transient_key = 'cta_unsubscribe_attempts_' . md5($ip);
 ...
 */
function cta_generate_newsletter_email_ajax() {
    check_ajax_referer('cta_ai_email', 'nonce');
    ...
    $context = cta_build_email_context($email_type, $course_id, $article_id, $custom_topic);
    ...
    // Build the prompt
    $prompt = cta_build_email_prompt($email_type, $context);
    
    // Call AI with email copywriting best practices from Email-copy.md
    $system_prompt = "You are an expert email copywriter for Continuity Training Academy, a care sector training provider in Kent, UK. Your goal is to generate a response: to move readers to take action.
...
- End with 'Best regards, The Continuity Training Academy Team'
...
Return a JSON object with exactly these fields:
{
  ...
  \"content\": \"Full HTML email content following 4 P's formula and all principles above\"
    ...
    $result = cta_ai_try_providers($preferred_provider, function($provider, $api_key) use ($system_prompt, $prompt) {
        if ($provider === 'groq') return cta_call_groq_api_newsletter($api_key, $system_prompt, $prompt);
        if ($provider === 'anthropic') return cta_call_anthropic_api_newsletter($api_key, $system_prompt, $prompt);
        return cta_call_openai_api_newsletter($api_key, $system_prompt, $prompt);
    });
    ...
    $parsed = cta_parse_ai_json_string($result);
...
add_action('wp_ajax_cta_generate_newsletter_email', 'cta_generate_newsletter_email_ajax');

/**
 * AJAX: Generate just the subject line
 */
function cta_generate_email_subject_ajax() {
    check_ajax_referer('cta_ai_email', 'nonce');
...
Email content:\n" . $content . "\n\nWrite only the subject line, nothing else. Make it compelling and benefit-focused.";
    ...
    $result = cta_ai_try_providers($preferred_provider, function($provider, $api_key) use ($subject_system, $prompt) {
        if ($provider === 'groq') return cta_call_groq_api_newsletter($api_key, $subject_system, $prompt);
        if ($provider === 'anthropic') return cta_call_anthropic_api_newsletter($api_key, $subject_system, $prompt);
        return cta_call_openai_api_newsletter($api_key, $subject_system, $prompt);
    });
...
add_action('wp_ajax_cta_generate_email_subject', 'cta_generate_email_subject_ajax');

/**
 * AJAX handler to get populated template content
 */
function cta_get_populated_template_ajax() {
    check_ajax_referer('cta_newsletter_nonce', 'nonce');
    ...
    $template_data = cta_get_populated_template($template_type);
...
add_action('wp_ajax_cta_get_populated_template', 'cta_get_populated_template_ajax');

/**
 * AJAX handler to get saved template content
 */
function cta_get_saved_template_ajax() {
    check_ajax_referer('cta_newsletter_nonce', 'nonce');
...
add_action('wp_ajax_cta_get_saved_template', 'cta_get_saved_template_ajax');

/**
 * AJAX: Create newsletter tag
 */
function cta_newsletter_create_tag_ajax() {
    check_ajax_referer('cta_newsletter_nonce', 'nonce');
    ...
    $result = cta_newsletter_create_tag($name, $color);
    if (is_wp_error($result)) {
...
add_action('wp_ajax_cta_newsletter_create_tag', 'cta_newsletter_create_tag_ajax');

/**
 * AJAX: Update subscriber tags
 */
function cta_newsletter_update_subscriber_tags_ajax() {
    check_ajax_referer('cta_newsletter_nonce', 'nonce');
    ...
    if (!empty($tag_ids)) {
        cta_newsletter_assign_tags([$subscriber_id], $tag_ids);
    }
...
add_action('wp_ajax_cta_newsletter_update_subscriber_tags', 'cta_newsletter_update_subscriber_tags_ajax');

/**
 * AJAX: Get recipients for tag ids (active subscribers only)
 */
function cta_newsletter_get_recipients_for_tags_ajax() {
    check_ajax_referer('cta_newsletter_nonce', 'nonce');
...
add_action('wp_ajax_cta_newsletter_get_recipients_for_tags', 'cta_newsletter_get_recipients_for_tags_ajax');

/**
 * Get populated template content with real data
 */
function cta_get_populated_template($template_type) {
    $base_templates = [
        'new_course' => [
            'subject' => 'New Course: [Course Name] - Pass Your Next CQC Inspection',
            'content' => '<p>Hi {first_name},</p>
...
<p>Best wishes,<br><strong>The Continuity Training Academy Team</strong></p>
<p style="margin-top: 24px; padding-top: 16px; border-top: 1px solid #e0e0e0; color: #646970; font-size: 13px;"><strong>P.S.</strong> This offer is valid for 30 days from your birthday. Don\'t miss out!</p>'
        ...
        'welcome' => [
            'subject' => 'Welcome to Continuity Training Academy!',
            'content' => '<p>Hi {first_name},</p>
<p><strong>Welcome to Continuity Training Academy!</strong></p>
...
<p style="margin-top: 24px; padding-top: 16px; border-top: 1px solid #e0e0e0; color: #646970; font-size: 13px;"><strong>P.S.</strong> This offer is exclusive to our newsletter subscribers. Thank you for being part of our community!</p>'
    ...
    switch ($template_type) {
        case 'upcoming_courses':
            $courses_html = cta_build_upcoming_courses_html();
            $template['content'] = str_replace('{upcoming_courses_list}', $courses_html, $template['content']);
            break;
            
        case 'quarterly':
            $summary_html = cta_build_quarterly_summary_html();
 ...
 */
function cta_build_upcoming_courses_html() {
 ...
 */
function cta_build_quarterly_summary_html() {
 ...
 */
function cta_build_email_context($type, $course_id, $article_id, $custom_topic) {
    $context = [];
 ...
 */
function cta_build_email_prompt($type, $context) {
 ...
 */
function cta_call_anthropic_api_newsletter($api_key, $system, $prompt) {
 ...
 */
function cta_call_openai_api_newsletter($api_key, $system, $prompt) {
 ...
 */
function cta_call_groq_api_newsletter($api_key, $system, $prompt) {
    $payload = [
 ...
 */
function cta_send_test_email($subject, $content, $link_type, $course_id, $article_id, $test_email) {
 ...
 */
function cta_add_email_to_queue($campaign_id, $subscriber_id, $email, $subject, $content, $headers, $scheduled_for = null) {
    global $wpdb;
    $queue_table = $wpdb->prefix . 'cta_email_queue';
 ...
 */
function cta_process_email_queue() {
    global $wpdb;
    $queue_table = $wpdb->prefix . 'cta_email_queue';
    $campaigns_table = $wpdb->prefix . 'cta_email_campaigns';
    $subscribers_table = $wpdb->prefix . 'cta_newsletter_subscribers';
    ...
    foreach ($pending as $queue_item) {
        ...
        $html_email = cta_build_email_template($queue_item->subject, $email_content, $site_name, $unsubscribe_url, $tracking_pixel_url);
    ...
    if ($more_pending > 0) {
        if (!wp_next_scheduled('cta_process_email_queue')) {
            wp_schedule_single_event(time() + 30, 'cta_process_email_queue');
        }
    }
}
add_action('cta_process_email_queue', 'cta_process_email_queue');

/**
 * Send welcome email to new subscriber
 */
function cta_send_welcome_email($subscriber_id, $email, $first_name = '') {
    // Check if welcome emails are enabled (can be disabled via option)
    $welcome_emails_enabled = get_option('cta_welcome_emails_enabled', true);
    ...
    if (!$welcome_template) {
        $subject = 'Welcome to Continuity Training Academy!';
        $content = cta_get_default_welcome_email_content();
    } else {
    ...
    $html_email = cta_build_email_template($subject, $content, $site_name, $unsubscribe_url, '');
    ...
    if (defined('WP_DEBUG') && WP_DEBUG) {
        if ($sent) {
            error_log('CTA Newsletter: Welcome email sent to ' . $email);
        } else {
            error_log('CTA Newsletter: Failed to send welcome email to ' . $email);
        }
    }
}
 ...
 */
function cta_get_default_welcome_email_content() {
 ...
 */
function cta_build_email_template($subject, $content, $site_name, $unsubscribe_url, $tracking_pixel_url) {
 ...
 */
function cta_retry_failed_emails($campaign_id = null) {
    global $wpdb;
    $queue_table = $wpdb->prefix . 'cta_email_queue';
 ...
 */
function cta_cancel_scheduled_campaign($campaign_id) {
    global $wpdb;
    $campaigns_table = $wpdb->prefix . 'cta_email_campaigns';
    $queue_table = $wpdb->prefix . 'cta_email_queue';
 ...
 */
function cta_send_birthday_emails() {
    global $wpdb;
    $table_name = $wpdb->prefix . 'cta_newsletter_subscribers';
    ...
    $template_data = cta_get_populated_template('birthday');
    ...
    foreach ($subscribers as $subscriber) {
        ...
        $html_email = cta_build_email_template($subject, $content, $site_name, $unsubscribe_url, $tracking_pixel_url);
...
add_action('cta_send_birthday_emails', 'cta_send_birthday_emails');

/**
 * Schedule daily birthday email check
 */
function cta_schedule_birthday_emails() {
    if (!wp_next_scheduled('cta_send_birthday_emails')) {
        // Schedule to run daily at 9 AM
        $schedule_time = strtotime('today 9:00 AM');
        if ($schedule_time < time()) {
            $schedule_time = strtotime('tomorrow 9:00 AM');
        }
        wp_schedule_event($schedule_time, 'daily', 'cta_send_birthday_emails');
    }
}
add_action('wp', 'cta_schedule_birthday_emails');
add_action('after_switch_theme', 'cta_schedule_birthday_emails');
 ...
 */
function cta_migrate_existing_data_to_newsletter() {
    global $wpdb;
    
    // Check if migration has already run
    $migration_key = 'cta_newsletter_migration_completed';
    ...
    foreach ($form_submissions as $submission) {
        ...
        if ($wpdb->last_error) {
            error_log('CTA Newsletter Migration: Error migrating form submission ' . $submission->ID . ' - ' . $wpdb->last_error);
            $skipped_count++;
        } else {
            $migrated_count++;
    ...
    if (defined('WP_DEBUG') && WP_DEBUG) {
        error_log(sprintf(
            'CTA Newsletter Migration: Migrated %d subscribers, skipped %d duplicates/existing',
            $migrated_count,
            $skipped_count
        ));
    }
 ...
 */
function cta_download_sample_csv() {
