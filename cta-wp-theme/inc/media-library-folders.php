 ...
 */
function cta_register_media_folders() {
    $labels = [
        'name' => 'Media Folders',
...
add_action('init', 'cta_register_media_folders');

/**
 * Add folder column to media library list view
 */
function cta_add_media_folder_column($columns) {
...
add_filter('manage_media_columns', 'cta_add_media_folder_column');

/**
 * Display folder in media library column
 */
function cta_display_media_folder_column($column_name, $post_id) {
...
add_action('manage_media_custom_column', 'cta_display_media_folder_column', 10, 2);

/**
 * Make folder column sortable
 */
function cta_make_media_folder_column_sortable($columns) {
    $columns['media_folder'] = 'media_folder';
    return $columns;
}
add_filter('manage_upload_sortable_columns', 'cta_make_media_folder_column_sortable');

/**
 * Handle sorting by folder
 */
function cta_sort_media_by_folder($query) {
...
add_action('pre_get_posts', 'cta_sort_media_by_folder');

/**
 * Add folder filter dropdown to media library
 */
function cta_add_media_folder_filter() {
    global $typenow;
    ...
    if ($typenow === 'attachment') {
        ...
        wp_dropdown_categories([
            'show_option_all' => sprintf(__('All %s', 'cta-theme'), $info_taxonomy->label),
            'taxonomy' => $taxonomy,
            'name' => $taxonomy,
            'orderby' => 'name',
            'selected' => $selected,
...
add_action('restrict_manage_posts', 'cta_add_media_folder_filter');

/**
 * Filter media library by folder
 */
function cta_filter_media_by_folder($query) {
...
add_action('parse_query', 'cta_filter_media_by_folder');

/**
 * Add folder meta box to attachment edit screen
 */
function cta_add_media_folder_meta_box() {
    add_meta_box(
        'media_folder_meta_box',
        'Media Folder',
        'cta_media_folder_meta_box_callback',
        'attachment',
        'side',
        'default'
    );
}
add_action('add_meta_boxes_attachment', 'cta_add_media_folder_meta_box');

/**
 * Media folder meta box callback
 */
function cta_media_folder_meta_box_callback($post) {
    wp_nonce_field('cta_media_folder_meta_box', 'cta_media_folder_meta_box_nonce');
 ...
 */
function cta_save_media_folder($post_id) {
    // Check nonce
    if (!isset($_POST['cta_media_folder_meta_box_nonce']) || 
        !wp_verify_nonce($_POST['cta_media_folder_meta_box_nonce'], 'cta_media_folder_meta_box')) {
        return;
    }
...
add_action('edit_attachment', 'cta_save_media_folder');

/**
 * Add folder selector to media uploader
 */
function cta_add_folder_to_media_uploader() {
    ?>
    <script>
...
add_action('print_media_templates', 'cta_add_folder_to_media_uploader');

/**
 * AJAX handler to get media folders
 */
function cta_get_media_folders_ajax() {
...
add_action('wp_ajax_cta_get_media_folders', 'cta_get_media_folders_ajax');

/**
 * Create default folders on theme activation
 */
function cta_create_default_media_folders() {
    $default_folders = [
        'Course Thumbnails',
        'Blog Posts',
        'About Us',
...
add_action('after_switch_theme', 'cta_create_default_media_folders');

/**
 * Add bulk actions for media library
 */
function cta_add_media_bulk_actions($actions) {
    $actions['assign_folder'] = 'Assign to Folder';
    return $actions;
}
add_filter('bulk_actions-upload', 'cta_add_media_bulk_actions');

/**
 * Handle bulk folder assignment
 */
function cta_handle_media_bulk_actions() {
...
add_action('admin_init', 'cta_handle_media_bulk_actions');

/**
 * Add bulk folder selector
 */
function cta_add_bulk_folder_selector() {
    global $typenow;
...
add_action('restrict_manage_posts', 'cta_add_bulk_folder_selector', 20);
