 ...
 */
function cta_limit_course_categories($post_id, $terms, $tt_ids, $taxonomy, $append, $old_tt_ids) {
    ...
    if (count($current_terms) > 2) {
        ...
        remove_action('set_object_terms', 'cta_limit_course_categories', 10, 6);
        
        // Set only the first 2 terms
        wp_set_object_terms($post_id, $limited_terms, 'course_category', false);
        
        // Re-add action
        add_action('set_object_terms', 'cta_limit_course_categories', 10, 6);
 ...
 */
function cta_get_primary_category($course_id) {
 ...
 */
function cta_get_secondary_category($course_id) {
    $terms = get_the_terms($course_id, 'course_category');
 ...
 */
function cta_limit_categories_admin_script() {
    global $post_type;
...
add_action('admin_footer', 'cta_limit_categories_admin_script');
 ...
 */
function cta_cleanup_course_categories() {
    $courses = get_posts([
        'post_type' => 'course',
        'posts_per_page' => -1,
        'post_status' => 'any',
 ...
 */
function cta_add_category_cleanup_action() {
    if (isset($_GET['cta_cleanup_categories']) && check_admin_referer('cta_cleanup_categories')) {
        $cleaned = cta_cleanup_course_categories();
...
add_action('admin_init', 'cta_add_category_cleanup_action');

/**
 * Add cleanup button to course categories admin page
 */
function cta_add_category_cleanup_button() {
    ...
    if ($pagenow === 'edit-tags.php' && $taxnow === 'course_category') {
        ...
        <div class="wrap" style="margin: 20px 0;">
            ...
            <p>
                <a href="<?php echo esc_url(wp_nonce_url(admin_url('edit-tags.php?taxonomy=course_category&cta_cleanup_categories=1'), 'cta_cleanup_categories')); ?>" 
...
add_action('admin_notices', 'cta_add_category_cleanup_button');
