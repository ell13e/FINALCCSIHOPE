 ...
 * Call this once via URL: http://cta.local/?create_missing_locations=1
 */
function cta_create_missing_location_pages() {
    // Security check
    if (!isset($_GET['create_missing_locations']) || !current_user_can('manage_options')) {
        return;
    }
    
    // Get the locations parent page
    $locations_parent = get_page_by_path('locations');
    if (!$locations_parent) {
        wp_die('Error: Locations parent page not found. Please create it first.');
    }
    
    // Define all UK location pages that should exist
    $location_pages = [
        [
    ...
    foreach ($location_pages as $page_data) {
        // Check if page already exists
        $full_slug = 'locations/' . $page_data['slug'];
        $existing = get_page_by_path($full_slug);
        
        if ($existing) {
            $skipped[] = $page_data['title'] . ' (already exists)';
            continue;
        }
    ...
    // Show results
    echo '<h1>Location Pages Created</h1>';
    
    if (!empty($created)) {
        echo '<h2 style="color: green;">✓ Created (' . count($created) . '):</h2>';
        echo '<ul>';
    ...
    if (!empty($skipped)) {
        echo '<h2>⊘ Skipped (' . count($skipped) . '):</h2>';
        echo '<ul>';
    ...
    echo '<p><a href="' . home_url('/locations/') . '">← Back to Locations</a></p>';
    exit;
}
add_action('init', 'cta_create_missing_location_pages');
