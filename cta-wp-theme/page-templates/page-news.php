    ...
    <div class="container">
      ...
      <div style="max-width: 600px; margin: 2rem auto 0;">
        <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
          ...
          <select id="news-category-filter" style="padding: 0.75rem; border: 1px solid #E8E3D6; border-radius: 4px; font-size: 0.9rem; background: white;">
            <option value="">All Categories</option>
            <option value="regulatory-updates">Regulatory Updates</option>
            <option value="training-tips">Training Tips & Best Practices</option>
            <option value="cqc-compliance">CQC Compliance</option>
            <option value="case-studies">Case Studies & Success Stories</option>
  ...
  <section class="news-articles-section">
    <div class="container">
      <?php if ($news_query->have_posts()) : 
        ...
        // Get all categories with post counts for filter buttons
        $all_categories = [];
        $category_counts = [];
      ...
      while ($news_query->have_posts()) : $news_query->the_post();
        ...
        if ($post_index === 1) :
          ...
          $featured_read_time = get_field('read_time') ?: cta_reading_time(get_the_content());
      ?>
      ...
      <div class="latest-articles-section">
        <div class="latest-articles-header">
          ...
          <div class="latest-articles-filters" role="group" aria-label="Filter articles by category">
            <button type="button" class="latest-articles-filter-btn active" data-category="all" aria-pressed="true">
              <span class="filter-btn-text">All</span>
              <span class="filter-btn-count" aria-label="Total articles"><?php echo count($news_query->posts); ?></span>
            </button>
            <?php foreach ($all_categories as $cat_name) : 
              $count = isset($category_counts[$cat_name]) ? $category_counts[$cat_name] : 0;
            ?>
            <button type="button" class="latest-articles-filter-btn" data-category="<?php echo esc_attr(strtolower($cat_name)); ?>" aria-pressed="false">
              <span class="filter-btn-text"><?php echo esc_html($cat_name); ?></span>
              <span class="filter-btn-count" aria-label="<?php echo esc_attr($cat_name); ?> articles"><?php echo $count; ?></span>
            </button>
            <?php endforeach; ?>
          </div>
        </div>
  ...
  <section class="cant-find-section">
    <div class="container">
      <div class="cant-find-content">
        <h2 class="cant-find-title">Ready to book training for your team?</h2>
        <p class="cant-find-description">Put the insights from these articles into practice with our CQC-compliant training courses.</p>
        <div style="display: flex; gap: 1rem; flex-wrap: wrap; justify-content: center; margin-top: 1.5rem;">
          <a href="<?php echo esc_url(get_post_type_archive_link('course_event')); ?>" class="btn btn-primary">View Course Calendar</a>
          <a href="<?php echo esc_url(get_permalink(get_page_by_path('group-training'))); ?>" class="btn btn-secondary">Request Group Training Quote</a>
        </div>
      </div>
    </div>
  </section>
...
(function() {
  ...
  // Function to count visible articles
  function countVisibleArticles() {
    return Array.from(articleCards).filter(function(card) {
