...
foreach ($resource_posts as $resource_post) {
    ...
    $icon = (string) get_post_meta($resource_id, '_cta_resource_icon', true);
    if ($icon === '') $icon = 'fas fa-file';

    $terms = get_the_terms($resource_id, 'cta_resource_category');
    $cat_slug = 'templates';
...
foreach (array_keys($resources_by_category) as $slug) {
    ...
    $term = get_term_by('slug', $slug, 'cta_resource_category');
