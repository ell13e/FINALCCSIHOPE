...
(function() {
  ...
  function handleSearchModalSubmit() {
    ...
    // Use the WordPress site URL from ctaData if available, otherwise use relative path
    const baseUrl = window.ctaData?.homeUrl || '/';
  ...
  async function getCourseSuggestionsAsync(query) {
    ...
    const ajaxUrl = window.ctaData?.ajaxUrl;
    if (!ajaxUrl) {
  ...
  // Course/Event Detail Page Accordions
  // ============================================
  
  /**
   * Standard accordion behavior for CTA “card” accordions used on Group Training / FAQs-style pages.
   *
   * Markup expectations:
   * - Button: .group-faq-question (aria-expanded, aria-controls)
   * - Panel:  .group-faq-answer   (id matches aria-controls, aria-hidden)
   */
  function initAccordions() {
    const buttons = document.querySelectorAll('.group-faq-question');
    if (buttons.length === 0) return;

    buttons.forEach((button) => {
      // Prevent double-binding if this gets called more than once.
  ...
  if (document.querySelector('.group-faq-question')) {
    if (document.readyState === 'loading') {
  ...
  if (document.querySelector('.course-detail-accordion-trigger')) {
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', initCourseDetailAccordions);
    } else {
